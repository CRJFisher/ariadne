...
- `packages/core/src/project/load_project.ts` — `load_project` now filters manifest entries to only include files present in the discovered file set, pruning stale entries for deleted files.
- `packages/core/src/persistence/persistence.test.ts` — Added integration test verifying that deleted files are pruned from the manifest after a warm reload.
