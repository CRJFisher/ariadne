...
- [x] #1 `@ariadnejs/skill-fs` package exists at `packages/skill-fs/`; both skills import `atomic_write_file`, `require_node_import_tsx`, and `error_code` from it; no duplicate copies remain under either `src/`
- [x] #2 `triage-entrypoints/src/` has top-level folders `dispense/`, `verdict/`, `absorb/`, `finalize/`, `store/`, `cross_run/`; the `coordinator/` folder no longer exists
- [x] #3 `triage-curator/src/` has top-level folders `absorb/`, `propose/`, `apply/`, `store/` (no `render/`; `render_ariadne_bug_body.ts` + `impact_report.ts` live in `propose/`; `render_authored_files.ts` lives in `apply/` because it writes to disk)
- [x] #4 `build_finalization_output.ts` is split into `finalize/output.ts` + `finalize/verdict_ledger.ts`; `merge_results.ts` and `finalize_triage.ts` both consume the shared loader (no duplicate `results/` walk)
- [x] #5 `paths.ts` and `triage_state_paths.ts` are merged into `store/paths.ts`; LATEST-pointer I/O lives in `store/latest_pointer.ts`
- [x] #6 `curator_drift_absorb.ts` is renamed to `drift_absorb.ts` (folder names the actor); no `curator_*` prefix survives under `triage-curator/src/`
- [x] #7 Agent-prompt pin tests live under `triage-curator/tests/`, not `src/` (only `agent_prompt_pin.test.ts` survives; the QA-prompt test was deleted with the QA wave in TASK-190.20.1)
- [x] #8 All 602 tests pass (386 triage-entrypoints + 208 triage-curator + 8 @ariadnejs/skill-fs); both skills' `tsc --noEmit` is clean; root `pnpm typecheck` covers skill-fs, types, core, mcp, triage-entrypoints, triage-curator in build order
- [x] #9 Every file move uses `git mv`; commit history preserves the rename trail
- [x] #10 No barrel re-exports from old paths; no deprecated aliases; importers updated in lockstep with each move
