...
which is in the sandbox `denyWithinAllow` list — correctly protecting skill _code_ from agent
modification, but inadvertently blocking all data _writes_ from the skill.
...
export const TRIAGE_PATTERNS_FILE = path.join(
  STATE_DIR,
