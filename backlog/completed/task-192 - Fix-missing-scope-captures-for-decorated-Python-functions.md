...
python.scm captures decorated function _definitions_ but never creates _scopes_ for them.
...
By contrast, decorated class _methods_ already work correctly (lines 40-47 of python.scm, with
explicit decorated_definition scope patterns). And decorated Python _classes_ also work because
...
Both handlers (handle_definition_function and handle_definition_function_async) are _identical_ —
...
Fixed two bugs in python.scm and capture_handlers.python.ts:

1. Added 2 missing @scope.function patterns for decorated module-level and nested Python functions (decorated_definition > function_definition). Previously, decorated functions had no body scope, causing "Could not find body scope" warnings.

2. Removed handle_definition_function_async() and all @definition.function.async patterns from python.scm that caused duplicate symbol creation for async functions (both the generic @definition.function and the async-specific pattern matched the same node).

Also fixed python_scope_boundary_extractor.test.ts to pass the outer decorated_node to extract_boundaries(), and added an end-to-end integration test in project.python.integration.test.ts verifying decorated function calls resolve correctly.
