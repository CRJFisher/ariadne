...
- [ ] #1 `scripts/propose_backlog_tasks.ts` exists; creates new backlog tasks for registry entries without a linked task and updates existing ones when `observed_count` changes
- [ ] #2 Tasks it creates carry the `group_id` as a label and include in the body: group description, example entry links, observed_count/observed_projects, proposed classifier spec, and a checklist of acceptance criteria
- [ ] #3 `scripts/generate_impact_report.ts` produces a markdown report with the 4 sections listed (top N, per-language, per-project, delta)
- [ ] #4 Golden-file test on a fixture curator state produces deterministic report output
