...
The plan engine's output target is the task-DB, not `backlog/`. This is the change to TASK-190.22.5's core: emit `PlanTask` rows via `JsonPlanTaskRepository` (190.22.5.1) and reconcile within the DB — replacing the old "emit via mcp\_\_backlog / reconcile against backlog/tasks" behavior.
...
- Grouping keys on `AriadneFaultArea` via `derive_fault_area` (190.22.4).
...
- [ ] #4 Any backlog access is read-only (dedup signal only) and passes the 190.22.5.2 firewall test
- [ ] #5 Grouping keys on `AriadneFaultArea` via `derive_fault_area` (190.22.4); smoke test over ≥2 runs green
<!-- AC:END -->
