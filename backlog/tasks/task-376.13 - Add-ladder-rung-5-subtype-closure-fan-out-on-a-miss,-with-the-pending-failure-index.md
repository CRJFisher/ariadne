...
- **Out of scope, recorded.**
  - A class-hit dispatch through `super` still fanned out to the parent's overrides, the calling class's own included (`super().save()` where the parent declares `save`), which AC #1 kept. TASK-376.20 replaced both `super` paths with a dispatch along the method resolution order.
