...
- [ ] #8 Pipeline resumes correctly after interruption: re-invoke `/triage` to resume (state file persists, hook re-activates)
...
2. Run triage: `npx tsx scripts/prepare_triage.ts` then invoke `/triage` skill
...
8. Test resumability: interrupt mid-triage, re-invoke `/triage`, verify continuation from state file
