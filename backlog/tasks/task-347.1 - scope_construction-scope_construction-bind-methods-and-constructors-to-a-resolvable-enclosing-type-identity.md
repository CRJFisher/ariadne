---
id: TASK-347.1
title: "[scope_construction] scope_construction: bind methods and constructors to a resolvable enclosing-type identity"
status: To Do
assignee: []
...
plan_source_task: pt-5674e699bfb6d650
---
...
All confirmed members in this bucket share one root cause: the lexical scope tree built by `process_scopes` does not associate a member scope (method or constructor) with the _type identity_ it belongs to. The bucket splits cleanly into two structurally distinct localized fixes plus an interim classifier mitigation:

- **Rust impl-block unification** — sibling `impl Foo` blocks must collapse to one `Foo` struct identity so intra- and cross-impl `self.method()` calls resolve.
- **Python constructor parenting** — every `__init__` / `cls(...)` / explicit-mixin construction must carry the owning class identity so a statically-known `ClassName(...)` call resolves to it.

The classifier node is the interim mitigation that suppresses these false-positives in triage while the two core fixes are built.
