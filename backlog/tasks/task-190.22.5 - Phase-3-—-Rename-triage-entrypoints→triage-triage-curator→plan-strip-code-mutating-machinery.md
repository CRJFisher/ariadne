...
- [ ] #2 `triage-curator-investigator.md` → `plan-strategist.md` (file rename only); `Task(...)` grants + `agent_prompt_pin.test.ts` updated to the new filename. The prompt-content rewrite to the strategist role is deferred to Phase 4 (190.22.9)
...
**Why this work exists.** The self-healing pipeline is being restructured into a clean `triage → plan` topology where `plan` is planning-only and all registry/file mutation belongs to a future, deferred actuator. This phase realises that topology **mechanically**: it renames the two skills and strips the curator's code-mutating machinery, deliberately staying a rename-shaped change so it reviews like a rename, not a feature. The new planning engine that will replace the stripped machinery is built separately in Phase 4 (190.22.9).

**The approach.** Two moves. (1) `git mv .claude/skills/triage-entrypoints → triage` and `triage-curator → plan` (plus `triage-curator-investigator.md → plan-strategist.md`, file rename only — the prompt's role rewrite is Phase 4), then update every name/path reference: each skill's `package.json` name, `tsconfig` include/paths, `plan`'s `meta.json` skill+path strings, the in-repo registry path `.claude/skills/triage/known_issues/registry.json` referenced from `packages/core` / `packages/types` / `packages/skill-fs`, and the `AUTO-GENERATED by …` provenance headers on the bundled core builtins. (2) Delete the code-mutating machinery from `plan` — `src/apply/*`, `src/propose/render_classifier.ts`, `scripts/sync_permanent_rules.ts`, `src/render_builtins_barrel.ts`, the promotion path, and the curator's drift/observation registry bookkeeping (`src/absorb/*` in the curator, distinct from the in-run coordinator that Phase 1 already deleted from triage). Because `scripts/finalize_run.ts` is the orchestrator whose entire job is to drive that machinery, the strip cascades into it: callers that import removed modules are pared back or removed so `plan`'s surviving golden path is read-and-propose only. `atomic_update_registry` stays in `@ariadnejs/skill-fs` for the future actuator but is no longer exercised by any golden path.

**What to know / watch.** Two intentional residuals stay behind and must NOT be renamed: the on-disk state-dir default `~/.ariadne/triage-entrypoints` in `skill-protocol/paths.ts` (renaming orphans existing state — a legacy-name comment is added) and historical `backlog/` docs. The rename can silently break the dead-code Stop hook and the script allowlists in `.claude/settings.json` / `.claude/hooks/detect_dead_code.ts`, so those are updated and verified by firing the hook and running a renamed `triage` script. The `.claude/rules/classifier-lifecycle.md` writer matrix is updated to record that `plan` no longer writes the registry. Completeness is proven by `grep -rIl 'triage-entrypoints|triage-curator'` returning only the two intentional-residual classes, and by a green `pnpm -r build && pnpm -r test`.
<!-- SECTION:NOTES:END -->
