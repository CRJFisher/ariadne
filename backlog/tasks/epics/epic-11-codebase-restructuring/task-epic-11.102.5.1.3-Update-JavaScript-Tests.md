...
describe("JavaScript Language Config", () => {
  ...
  describe("Class Definitions", () => {
    it("should create class definition with all fields", () => {
      ...
      JAVASCRIPT_BUILDER_CONFIG.get("def.class")!.process(
        capture,
        builder,
        context
      );
  ...
  describe("Function Definitions", () => {
    it("should create function definition with signature", () => {
      ...
      JAVASCRIPT_BUILDER_CONFIG.get("def.function")!.process(
        capture,
        builder,
        context
      );
  ...
  describe("Variable Definitions", () => {
    it("should distinguish const from let/var", () => {
      const constCapture: RawCapture = {
        ...
        node: parse_javascript(
          "const MY_CONST = 42;"
        ).rootNode.descendantForPosition({ row: 0, column: 6 }),
        capture_name: "def.variable",
      };

      JAVASCRIPT_BUILDER_CONFIG.get("def.variable")!.process(
        constCapture,
        builder,
        context
      );
  ...
  describe("Import Definitions", () => {
    it("should handle default imports", () => {
      ...
      JAVASCRIPT_BUILDER_CONFIG.get("def.import")!.process(
        capture,
        builder,
        context
      );
