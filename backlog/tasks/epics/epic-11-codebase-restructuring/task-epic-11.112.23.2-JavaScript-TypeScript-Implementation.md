 ...
 */
function analyze_export_statement(
  ...
  const has_from = export_node.children.some((child) => child.type === "from");
...
export function extract_export_info(
  node: SyntaxNode,
  symbol_name?: SymbolName
): { is_exported: boolean; export?: ExportMetadata };
```
...
**Updated all add\_\* methods to accept and store export fields:**
