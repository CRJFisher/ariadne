...
- [ ] #1 `Project` + `update_file`: a `use`-imported `Parker::new()` resolves to the `new` associated function (the `new` definition is reachable, not an entry point), not merely to the `Parker` class symbol.
- [ ] #2 `Self::new()` inside an `impl` resolves to the enclosing type's `new` constructor.
- [ ] #3 `new` is not double-counted (a struct with one `fn new` exposes exactly one constructor target; no duplicate reachable symbols).
- [ ] #4 Existing constructor-resolution tests for TypeScript/Python stay green — the change is gated to the Rust associated-constructor path and does not alter the TS `[namespace, class]` `property_chain` branch.
- [ ] #5 Integration tests over fixtures reproducing the cluster's `Type::new()` evidence cases demonstrate each constructor resolves (no longer an entry point); fixture additions/updates accompany any changed call-graph expectations.
