...
## Acceptance Criteria
<!-- AC:BEGIN -->
- [x] #1 `.claude/agents/triage-investigator.md` updated with the 4-step residual-only strategy and `novel:` prefix convention
- [x] #2 `scripts/update_registry_from_triage.ts` exists and is invoked from `finalize_aggregation.ts` (or runs standalone)
- [x] #3 Dry-run test: running on fresh project with a novel group_id emitted ≥5 times results in a new registry entry with `status: "wip"` and `classifier.kind: "none"`
...
- [x] #6 Agent prompt explicitly instructs reading `packages/core/src/index_single_file/query_code_tree/queries/unsupported_features.{lang}.md` when captures are missing
- [x] #7 Feedback loop never auto-writes a non-`kind: "none"` `ClassifierSpec`; promotion from `wip` to an active classifier is always a human-reviewed PR
<!-- AC:END -->
