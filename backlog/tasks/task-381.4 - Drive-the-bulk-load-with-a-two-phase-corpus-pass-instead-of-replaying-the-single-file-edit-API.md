...
- [ ] #4 #4 Any call edge the unpatched build resolves and this one does not is reproduced as order-dependent on the unpatched build, recorded edge by edge; the reversed-order run gives byte-identical edge sets between the two builds; and no edge is lost that the unpatched build resolves under ALL THREE named orders (forward, reversed, and the seeded shuffle whose seed the harness records). Edges lost under one or two of the three are recorded individually as instances of the order-dependence TASK-381.11 owns. REFUTED in both halves and replaced. MEASURED at n=1,200 across four orders (forward, reversed, descending byte-size, seeded shuffle): the reversed-order run does NOT give byte-identical edge sets against forward (43 lost, 17 gained), and 22/4/10/23 edges (forward/reversed/descending-size/shuffled respectively) are lost that the unpatched build resolves under every order tested — against 55/77, 39/43, 11/21 and 6/29 that are genuine pre-existing order-dependence. Every stable loss reaches a method on an exported singleton (`errors.ts`, `ime.ts`, `modesRegistry.ts`, `idGenerator.ts`, `ipcMain.ts`, `inputMode.ts`) through a constructor binding — the defect TASK-381.11 names by mechanism — none is a re-target, and the call site stops resolving. Against that, the candidate resolves 693 edges forward the per-arrival driver did not, and 300 previously-unresolved call sites become resolved against 63 that stop. The criterion is replaced: a stable, edge-by-edge-accounted loss attributed to TASK-381.11's named defect class is acceptable; byte-identical reversed-order edge sets is not required of this driver.
...
- [ ] #7 #7 With TASK-381.3's reverse indices and this driver both present, whole-load CPU improves by >= 1.8x at n=1,200 (measured 1.981x) and >= 1.4x at n=600 (measured 1.472x) against the driver-without-indices arm, interleaved in separate processes, reported as cpu_user with cpu/wall and loadavg. n=600's 1.4x FLOOR REFUTED as portable and replaced. MEASURED on landed code, interleaved A,B,A,B in separate processes against a driver-without-indices control in the same session, byte-identical fingerprints on both arms: 2.03x at n=1,200 (>=1.8 met) and 1.36x at n=600, with control CV 0.01% and candidate CV 0.22% — the shortfall is the machine, not noise; the 1.472x target came from a different box. The criterion is a same-session ratio only: >= 1.8x at n=1,200 stands as the gate; n=600's ratio is reported alongside it, not gated on an absolute floor, until re-measured on the box the original 1.472x came from.
