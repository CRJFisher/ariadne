...
- **Possible end-state: no Backlog.md.** The task-DB + a thin ordering/curation layer becomes the planning surface; the export adapter (190.22.11) either targets the user's chosen external tool or is retired. The "switch planning tools easily" seam exists precisely so this is a low-cost pivot.
