...
describe("cli_args", () => {
  describe("parse_cli_args", () => {
    ...
    it("returns empty object for unrelated arguments", () => {
      const result = parse_cli_args(["--verbose", "--debug"]);
      expect(result).toEqual({});
    });

    it("handles --project-path as last argument without value", () => {
      const result = parse_cli_args(["--project-path"]);
      expect(result).toEqual({ project_path: undefined });
    });

    it("uses last matching flag when multiple provided", () => {
    ...
    it("handles path with spaces in equals format", () => {
    ...
    it("returns undefined toolsets when not specified", () => {
      const result = parse_cli_args([]);
