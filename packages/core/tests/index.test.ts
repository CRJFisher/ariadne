...
describe("Project - Function discovery APIs", () => {
  ...
  test("get_all_functions filters private functions", () => {
    ...
    const withoutPrivate = project.get_all_functions({
      include_private: false,
    });

    const withPrivateNames =
      withPrivate.get("test.ts")?.map((f) => f.name) || [];
    const withoutPrivateNames =
      withoutPrivate.get("test.ts")?.map((f) => f.name) || [];
  ...
  test("get_all_functions filters test functions", () => {
    ...
    const withoutTestNames =
      withoutTests.get("test.ts")?.map((f) => f.name) || [];
  ...
  test("get_all_functions respects symbol_kinds filter", () => {
    ...
    const onlyFunctions = project.get_all_functions({
      symbol_kinds: ["function"],
    });
    const onlyMethods = project.get_all_functions({ symbol_kinds: ["method"] });
    const onlyGenerators = project.get_all_functions({
      symbol_kinds: ["generator"],
    });

    const functionNames =
      onlyFunctions.get("test.ts")?.map((f) => f.name) || [];
    const methodNames = onlyMethods.get("test.ts")?.map((f) => f.name) || [];
    const generatorNames =
      onlyGenerators.get("test.ts")?.map((f) => f.name) || [];
...
describe("get_definitions API", () => {
  test("should return all function definitions in a TypeScript file", () => {
    ...
    expect(functionDefs.map((d) => d.name).sort()).toEqual([
      "func1",
      "func2",
      "func3",
      "func4",
  ...
  test("should return class definitions", () => {
    ...
    expect(classes.map((d) => d.name).sort()).toEqual([
      "AbstractClass",
      "ExportedClass",
      "MyClass",
    ]);
  ...
  test("should work with JavaScript files", () => {
    ...
    expect(defs.find((d) => d.name === "arrowFunc")?.symbol_kind).toBe(
      "constant"
    );
  });
  ...
  test("should include enclosing ranges when available", () => {
    ...
    if (func?.enclosing_range) {
      expect(func.enclosing_range.start.row).toBeLessThan(
        func.enclosing_range.end.row
      );
    }
  });
