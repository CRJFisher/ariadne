...
import { get_language_parser } from '../src/scope_queries/loader';
import { build_scope_tree } from '../src/scope_analysis/scope_tree';
import {
  ...
  find_all_references,
  go_to_definition
} from '../src/scope_analysis/symbol_resolution';
...
describe('Symbol Resolution', () => {
  describe('JavaScript', () => {
    ...
    it('should resolve local variables', () => {
      ...
      const context = create_resolution_context(scope_tree, language, 'test.js', tree.rootNode, code);
      
      // Find the block scope inside the function (where x is defined)
      const block_scope = Array.from(scope_tree.nodes.values()).find(s => s.type === 'block' && s.symbols.has('x'));
      expect(block_scope).toBeDefined();
      
      // Resolve 'x' from within the block scope
      const resolved = resolve_symbol_with_language('x', block_scope!.id, context, language);
    ...
    it('should resolve hoisted variables', () => {
      ...
      const func_scope = Array.from(scope_tree.nodes.values()).find(s => s.type === 'function');
      expect(func_scope).toBeDefined();
      
      // Should resolve 'x' even though it's accessed before declaration
      const resolved = resolve_symbol_with_language('x', func_scope!.id, context, language);
      expect(resolved).toBeDefined();
      expect(resolved?.symbol.name).toBe('x');
    });
  ...
  describe('TypeScript', () => {
    ...
    it('should resolve type parameters', () => {
      ...
      const tree = parser.parse(code);
      const scope_tree = build_scope_tree(tree.rootNode, code, language);
      const context = create_resolution_context(scope_tree, language, 'test.ts', tree.rootNode, code);
...
outer()
      ...
      const tree = parser.parse(code);
      const scope_tree = build_scope_tree(tree.rootNode, code, language);
      const context = create_resolution_context(scope_tree, language, 'test.py', tree.rootNode, code);
      ...
      if (inner_scope) {
        const resolved = resolve_symbol_with_language('x', inner_scope.id, context, language);
        expect(resolved).toBeDefined();
...
from ..core import *
      ...
      expect(imports.some(i => i.name === 'system' && i.source_name === 'sys')).toBe(true);
      expect(imports.some(i => i.name === 'Path')).toBe(true);
      expect(imports.some(i => i.name === 'Dictionary' && i.source_name === 'Dict')).toBe(true);
    });
  });
...
fn main() {
    let map: HashMap<String, i32> = HashMap::new();
      ...
      const tree = parser.parse(code);
      const scope_tree = build_scope_tree(tree.rootNode, code, language);
      const context = create_resolution_context(scope_tree, language, 'test.rs', tree.rootNode, code);
      ...
      if (main_scope) {
        ...
        const resolved = resolve_symbol_with_language('HashMap', main_scope.id, context, language);
        expect(resolved).toBeDefined();
...
impl MyStruct {
    ...
    fn get_value(&self) -> i32 {
      ...
      if (new_method) {
        ...
        const self_resolved = resolve_symbol_with_language('Self', new_method.id, context, language);
      ...
      if (get_method) {
        ...
        const self_resolved = resolve_symbol_with_language('self', get_method.id, context, language);
        expect(self_resolved).toBeDefined();
...
pub use crate::config::Config;
  ...
  describe('Cross-language features', () => {
    it('should find references within a file', () => {
      ...
      const tree = parser.parse(code);
      const scope_tree = build_scope_tree(tree.rootNode, code, language);
      const refs = find_all_references('value', scope_tree, language, 'test.js', tree.rootNode, code);
    ...
    it('should go to definition', () => {
      ...
      // Debug: Check if myFunction is in the scope tree
      console.log('Root scope symbols:', Array.from(root_scope!.symbols.entries()));
      console.log('All scopes:', Array.from(scope_tree.nodes.values()).map(s => ({
        id: s.id,
        type: s.type,
        metadata: s.metadata,
