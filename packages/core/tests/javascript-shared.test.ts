...
import { Def, Import, Project, Ref } from "../src/src/index";
import {
  ...
  return response.json();
    ...
    test: (project, fileName) => {
      ...
      const funcDef = defs.find(
        (d) => d.name === "fetchData" && d.symbol_kind === "function"
      );
...
function greet(greeting, ...names) {
  return \`\${greeting} \${names.join(', ')}\`;
    ...
    test: (project, fileName) => {
      ...
      expect(greetFunc!.metadata!.parameter_names).toEqual([
        "greeting",
        "...names",
      ]);
    },
  ...
  for (let i = start; i <= end; i++) {
    ...
    test: (project, fileName) => {
      ...
      const genFunc = defs.find(
        (d) => d.name === "numberGenerator" && d.symbol_kind === "generator"
      );
      expect(genFunc).toBeDefined();
