...
export function build_call_graph_for_display(
  ...
  // Filter out private Rust methods that are never called AND make no calls
  // Private Rust methods (without 'pub') shouldn't appear in the call graph if they're isolated
  ...
  for (const [nodeId, node] of nodes) {
    if (node.definition.file_path.endsWith('.rs') && 
        ...
        node.calls.length === 0 &&  // Only remove if it makes no calls
