...
import { Range } from "@ariadnejs/types";

// Re-export SimpleRange for convenience
export type { Range as SimpleRange } from "@ariadnejs/types";
 ...
 */
export function find_definition_range(def: Def, fileCache: FileCache): Range {
  ...
  if (defNode) {
    ...
    while (current) {
      ...
      // Check for variable/const nodes
        ...
        ["variable", "const", "let", "constant"].includes(def.symbol_kind)
      ) {
        if (
      ...
      if (foundDefinitionNode) {
        definitionRange = {
          start: {
            row: current.startPosition.row,
            column: current.startPosition.column,
          },
          end: {
            row: current.endPosition.row,
