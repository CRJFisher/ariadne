...
import { TypeDiscovery, Range } from "@ariadnejs/types";
import { get_local_variable_type, LocalTypeTrackerData } from "../type_tracker";
import {
...
export function resolve_method_call_pure(
  ...
  const typeInfo = get_local_variable_type(
    localTypeTracker,
    objectName,
    ref.range.start
  );
  ...
  if (typeInfo && typeInfo.classDef) {
    ...
    if (config.get_file_graph && typeInfo.classDef.file_path) {
      ...
      const classDefs = classGraph
        ? classGraph.getNodes<Def>("definition")
        : [];

      // Compute enclosing_range if not available
      let classRange: Range | undefined = (typeInfo.classDef as any)
        .enclosing_range;
      if (!classRange && config.get_file_cache) {
        const classFileCache = config.get_file_cache(
          typeInfo.classDef.file_path
        );
        if (classFileCache) {
          classRange = compute_class_enclosing_range(
            typeInfo.classDef,
            classFileCache.tree
          );
        }
      ...
      const expectedSymbolId = `${typeInfo.classDef.file_path.replace(
        ".rs",
        ""
      )}#${typeInfo.className}.${methodName}`;

      const method = classDefs.find(
        (m: Def) =>
          m.name === methodName &&
...
export function resolve_method_on_type(
  ...
  const typeDef = currentGraph
    .getNodes<Def>("definition")
    .find(
      (d) =>
        d.name === typeName &&
...
export function is_method_of_class(
  ...
  let classRange: Range | undefined = (classDef as any).enclosing_range;
  ...
  return (
    is_position_within_range(methodDef.range.start, classRange) &&
