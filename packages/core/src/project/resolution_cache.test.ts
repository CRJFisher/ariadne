...
describe("ResolutionCache", () => {
  ...
  describe("remove_file", () => {
    ...
    it("should remove resolutions when file has active resolutions", () => {
      const file1 = "file1.ts" as FilePath;
      const file2 = "file2.ts" as FilePath;
      const ref1 = reference_id("foo", { file_path: file1, start_line: 5, start_column: 10, end_line: 5, end_column: 13 });
      const ref2 = reference_id("bar", { file_path: file2, start_line: 6, start_column: 10, end_line: 6, end_column: 13 });
      const symbol1 = function_symbol("foo", { file_path: file1, start_line: 1, start_column: 0, end_line: 3, end_column: 1 });
      const symbol2 = function_symbol("bar", { file_path: file2, start_line: 2, start_column: 0, end_line: 4, end_column: 1 });

      cache.set(ref1, symbol1, file1);
      cache.set(ref2, symbol2, file2);

      expect(cache.size()).toBe(2);
      expect(cache.get(ref1)).toBe(symbol1);
      expect(cache.get(ref2)).toBe(symbol2);

      // Remove file1 without invalidating first
      cache.remove_file(file1);
  ...
  describe("get_all_referenced_symbols", () => {
    it("should return all unique referenced symbols", () => {
      ...
      const ref_id1 = reference_id("foo", { file_path: file1, start_line: 5, start_column: 10, end_line: 5, end_column: 13 });
      const ref_id2 = reference_id("bar", { file_path: file1, start_line: 6, start_column: 10, end_line: 6, end_column: 13 });
      const ref_id3 = reference_id("foo", { file_path: file2, start_line: 3, start_column: 5, end_line: 3, end_column: 8 });

      const symbol_A = function_symbol("foo", { file_path: file1, start_line: 1, start_column: 0, end_line: 3, end_column: 1 });
      const symbol_B = function_symbol("bar", { file_path: file1, start_line: 4, start_column: 0, end_line: 6, end_column: 1 });

      cache.set(ref_id1, symbol_A, file1);
      cache.set(ref_id2, symbol_B, file1);
      cache.set(ref_id3, symbol_A, file2);  // Duplicate symbol_A
