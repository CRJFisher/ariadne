...
import type { FilePath, SymbolName, SymbolId, Location, TypeId, LocationKey } from "@ariadnejs/types";
...
import { analyze_type_flow, type ResolvedTypeFlow } from "./type_resolution/type_flow";
import type { LocalTypeFlowData, LocalAssignmentFlow, LocalReturnFlow, FlowSource } from "../semantic_index/references/type_flow_references";
...
export function analyze_integrated_type_flow(
  ...
  const prepared_flows = convert_flows_for_analysis(local_extraction.type_flows);
 ...
 */
function convert_to_integrated_flow(resolved: ResolvedTypeFlow): IntegratedTypeFlow {
