...
import { find_containing_scope } from "../../semantic_index/scope_tree";
...
function narrow_by_sibling_methods(
  ...
  context: EnhancedMethodResolutionContext,
  method_index: MethodIndex,
  candidate_methods?: Array<{ class_id: SymbolId; method_id: SymbolId; is_static?: boolean }>
): EnhancedResolution | null {
...
function find_scope_for_location(
  ...
  const root_scope = context.current_index.scopes.get(context.current_index.root_scope_id);
  if (!root_scope) return null;

  const scope = find_containing_scope(location, root_scope, context.current_index.scopes);
  return scope.id;
}
