...
import { build_method_index, resolve_method_heuristic, ResolutionStrategy } from "./heuristic_resolver";
...
import type { ClassDefinition, MethodDefinition } from "@ariadnejs/types/src/symbol_definitions";
...
describe("Heuristic Method Resolution (Simple)", () => {
  ...
  const get_name_method = method_symbol("getName", "User", "/test.ts", test_location);
  ...
  it("should resolve unique method names directly", () => {
    ...
    const result = resolve_method_heuristic(member_access, context, method_index);
  ...
  it("should return null when multiple methods exist without disambiguation", () => {
    ...
    const admin_get_name = method_symbol("getName", "Admin", "/test.ts", test_location);
