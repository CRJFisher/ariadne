...
import type { FilePath, SymbolName, Location } from "@ariadnejs/types";
import {
  mock_location,
  mock_semantic_index,
...
describe("Test Factories", () => {
  ...
  describe("Advanced ReadonlyMap Utilities", () => {
    ...
    describe("NestedReadonlyMapBuilder", () => {
      it("should build nested ReadonlyMap structures", () => {
        ...
        const result = builder
          .set_nested("group1", [["a", 1], ["b", 2]])
          .set_nested("group2", [["x", 10], ["y", 20]])
          .build();
      ...
      it("should work with ReadonlyMapBuilder for nested construction", () => {
        ...
        const main_builder = new NestedReadonlyMapBuilder<string, string, number>();
    ...
    describe("Type Resolution Builder", () => {
      it("should create properly structured type resolution data", () => {
        ...
        builders.symbol_types.set("symbol1" as any, "type1" as any);
        builders.type_members.set_nested("type1" as any, [
          ["member1" as SymbolName, "member_symbol1" as any],
          ["member2" as SymbolName, "member_symbol2" as any]
        ]);
        builders.constructors.set("type1" as any, "constructor_symbol" as any);
        ...
        expect(result.symbol_types.get("symbol1" as any)).toBe("type1");
        expect(result.type_members.get("type1" as any)?.get("member1" as SymbolName)).toBe("member_symbol1");
        expect(result.constructors.get("type1" as any)).toBe("constructor_symbol");
      });
    });
    ...
    describe("Import Resolution Builder", () => {
      it("should create properly structured import resolution data", () => {
        ...
        builders.imports.set_nested("file1.ts" as FilePath, [
          ["import1" as SymbolName, "symbol1" as any],
          ["import2" as SymbolName, "symbol2" as any]
        ]);
        builders.unresolved_imports.set("location1" as any, "unresolved" as SymbolName);
        ...
        expect(result.imports.get("file1.ts" as FilePath)?.get("import1" as SymbolName)).toBe("symbol1");
        expect(result.unresolved_imports.get("location1" as any)).toBe("unresolved");
      });
    });

    describe("Immutable Update Utilities", () => {
      it("should update ReadonlyMap immutably", () => {
        const original = readonly_map_from_entries([["a", 1], ["b", 2]]);
        const updated = update_readonly_map(original, [["b", 20], ["c", 3]]);
      ...
      it("should update nested ReadonlyMap immutably", () => {
        ...
        const updated = update_nested_readonly_map(original, "group1", [["b", 20], ["c", 3]]);
  ...
  describe("Project-level Factories", () => {
    ...
    it("should create ResolutionInput", () => {
      const files = new Map([
        [test_file, mock_semantic_index(test_file)]
      ]);
