 ...
 */
function extract_expression_name(node: SyntaxNode, source_code: string): string | null {
  if (node.type === 'identifier') {
...
function detect_abstract_base_class(
  ...
  if (info.parent_class && abc_patterns.some(p => info.parent_class!.includes(p))) {
 ...
 */
function extract_decorator_name(decorator: SyntaxNode, source_code: string): string | null {
  // Skip the @ symbol
 ...
 */
export function is_abstract_class_python(
  info: ClassInfo
): boolean {
...
export function extract_abstract_methods_python(
  ...
  for (let i = 0; i < body.childCount; i++) {
    ...
    if (statement.type === 'function_definition') {
      ...
      if (decorators.some(d => d === 'abstractmethod' || d.endsWith('.abstractmethod'))) {
...
export function compute_python_mro(
  ...
  for (const base of bases) {
    if (!visited.has(base)) {
      ...
      for (const ancestor of base_mro.slice(1)) { // Skip the base itself
