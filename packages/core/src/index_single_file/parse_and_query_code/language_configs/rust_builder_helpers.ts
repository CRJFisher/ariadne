...
import type { RawCapture } from "../scope_processor";

// ============================================================================
// Additional Symbol Creation Functions (not in @ariadnejs/types yet)
// ============================================================================

function symbol_string(kind: string, name: SymbolName, location: Location): SymbolId {
  const parts = [
    kind,
    location.file_path,
    location.line,
...
export function extract_symbol_name(capture: RawCapture): SymbolName {
...
export function create_struct_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
  return class_symbol(name, location);
}

export function create_enum_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
  return enum_symbol(name, location);
}

export function create_trait_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
  return interface_symbol(name, location);
}

export function create_function_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
  return function_symbol(name, location);
}

export function create_method_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
  return method_symbol(name, location);
}

export function create_field_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
  return property_symbol(name, location);
}

export function create_variable_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
  return variable_symbol(name, location);
}

export function create_constant_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
  return constant_symbol(name, location);
}

export function create_parameter_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
  return parameter_symbol(name, location);
}

export function create_module_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
  return module_symbol(name, location);
}

export function create_type_alias_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
...
export function extract_visibility(node: SyntaxNode): SymbolAvailability {
  ...
  while (testNode) {
    const visibilityNode = testNode.children?.find(child => child.type === 'visibility_modifier');
...
export function is_const_function(node: SyntaxNode): boolean {
  ...
  return node.children?.some(child => child.type === 'const' || child.text === 'const') || false;
}

export function is_unsafe_function(node: SyntaxNode): boolean {
  // Check for unsafe keyword
  return node.children?.some(child => child.type === 'unsafe' || child.text === 'unsafe') || false;
}
...
export function extract_return_type(node: SyntaxNode): SymbolName | undefined {
  ...
  if (returnType) {
    ...
    const typeNode = returnType.children?.find(child =>
      child.type !== '->' && child.type !== 'type');
    return (typeNode?.text || returnType.text?.replace('->', '').trim()) as SymbolName;
  }
  return undefined;
}
...
export function is_mutable_parameter(node: SyntaxNode): boolean {
  ...
  if (pattern) {
    return pattern.children?.some(child => child.type === 'mut' || child.text === 'mut') || false;
  }
  return false;
}
...
export function is_self_parameter(node: SyntaxNode): boolean {
  const pattern = node.childForFieldName?.('pattern');
  if (pattern) {
    return pattern.type === 'self' || pattern.text === 'self' ||
           pattern.text === '&self' || pattern.text === '&mut self';
  }
  return false;
}

export function find_containing_impl(capture: RawCapture): { struct?: SymbolId, trait?: SymbolId } | undefined {
...
export function find_containing_struct(capture: RawCapture): SymbolId | undefined {
  let node = capture.node;
  ...
  while (node) {
    if (node.type === 'struct_item') {
      ...
      if (nameNode) {
        return class_symbol(nameNode.text as SymbolName, extract_location(nameNode));
      }
    }
    node = node.parent;
  }
...
export function find_containing_enum(capture: RawCapture): SymbolId | undefined {
  let node = capture.node;
  ...
  while (node) {
    if (node.type === 'enum_item') {
      const nameNode = node.childForFieldName?.('name');
      if (nameNode) {
        return enum_symbol(nameNode.text as SymbolName, extract_location(nameNode));
      }
    }
    node = node.parent;
  }
...
export function find_containing_trait(capture: RawCapture): SymbolId | undefined {
  let node = capture.node;
  ...
  while (node) {
    if (node.type === 'trait_item') {
      const nameNode = node.childForFieldName?.('name');
      if (nameNode) {
        return interface_symbol(nameNode.text as SymbolName, extract_location(nameNode));
      }
    }
    node = node.parent;
  }
...
export function find_containing_module(capture: RawCapture): SymbolId | undefined {
  let node = capture.node;
  ...
  while (node) {
    if (node.type === 'mod_item') {
      const nameNode = node.childForFieldName?.('name');
      if (nameNode) {
        return module_symbol(nameNode.text as SymbolName, extract_location(nameNode));
      }
    }
    node = node.parent;
  }
    ...
    if (child.type === 'enum_variant_list') {
      for (const variant of child.children || []) {
        if (variant.type === 'enum_variant') {
          ...
          const nameNode = variant.children?.find(c => c.type === 'identifier');
          if (nameNode) {
