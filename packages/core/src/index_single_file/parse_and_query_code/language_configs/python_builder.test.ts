...
describe("Python Builder Configuration", () => {
  ...
  describe("PYTHON_BUILDER_CONFIG", () => {
    ...
    pass`;
      const capture = createCapture(code, "def.param.typed.default", "identifier");
      const builder = new DefinitionBuilder(createTestContext());
    ...
    pass`;
      ...
      // Test *args
      const argsCapture = createCapture(code, "def.param.args", "list_splat_pattern");
      ...
      // Test **kwargs
      const kwargsCapture = createCapture(code, "def.param.kwargs", "dictionary_splat_pattern");
    ...
    it("should handle aliased imports", () => {
      ...
      const capture = createCapture(code, "import.module.source", "dotted_name");
      const builder = new DefinitionBuilder(createTestContext());
    ...
    def add(self, a, b):
        ...
        if (className) {
          ...
          PYTHON_BUILDER_CONFIG.get("def.class")?.process(classCapture, builder, context);
        }
        ...
        if (methodName) {
          ...
          PYTHON_BUILDER_CONFIG.get("def.method")?.process(methodCapture, builder, context);
        }
    ...
    return f"Hello {name}"`;
        ...
        if (funcName) {
          ...
          PYTHON_BUILDER_CONFIG.get("def.function")?.process(funcCapture, builder, context);
        }
      ...
      it("should distinguish between constants and variables by naming convention", () => {
        ...
        if (constNode) {
          ...
          PYTHON_BUILDER_CONFIG.get("def.variable")?.process(constCapture, builder1, context);
        }
        ...
        if (varNode) {
          const varCapture: RawCapture = {
            name: "def.variable",
            node: varNode as any,
            text: varNode.text,
          };
          PYTHON_BUILDER_CONFIG.get("def.variable")?.process(varCapture, builder2, context);
        }
      ...
      it("should handle complex import patterns", () => {
        ...
        for (const id of identifiers) {
          if (id.text === "List" || id.text === "Dict" || id.text === "Optional") {
            const importCapture: RawCapture = {
              name: "import.named",
              node: id as any,
              text: id.text,
            };
            PYTHON_BUILDER_CONFIG.get("import.named")?.process(importCapture, builder, context);
          }
        }
