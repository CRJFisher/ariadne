...
import { JAVASCRIPT_BUILDER_CONFIG, type ProcessFunction, type LanguageBuilderConfig } from "./javascript_builder";
 ...
 */
function create_interface_id(capture: RawCapture): SymbolId {
 ...
 */
function create_type_alias_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
 ...
 */
function create_enum_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
 ...
 */
function create_namespace_id(capture: RawCapture): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
 ...
 */
function create_enum_member_id(capture: RawCapture, enum_id: SymbolId): SymbolId {
  const name = extract_symbol_name(capture);
 ...
 */
function create_method_signature_id(capture: RawCapture, interface_name: SymbolName): SymbolId {
  const name = extract_symbol_name(capture);
  const location = extract_location(capture.node);
 ...
 */
function find_containing_interface(capture: RawCapture): SymbolId | undefined {
  let node = capture.node.parent;
 ...
 */
function find_containing_enum(capture: RawCapture): SymbolId | undefined {
  let node = capture.node.parent;
  ...
  while (node) {
    if (node.type === 'enum_declaration') {
      ...
      if (nameNode) {
        ...
        return `enum:${location.file_path}:${location.line}:${location.column}:${location.end_line}:${location.end_column}:${enumName}` as SymbolId;
      }
    }
    node = node.parent;
  }
 ...
 */
function find_decorator_target(capture: RawCapture): SymbolId | undefined {
  ...
  if (parent) {
    ...
    if (decoratorIndex !== undefined && decoratorIndex >= 0) {
      ...
      for (let i = decoratorIndex + 1; i < (parent.children?.length || 0); i++) {
        const sibling = parent.children![i];
        if (sibling.type !== 'decorator') {
          // Found the target - extract its ID based on type
          if (sibling.type === 'class_declaration' || sibling.type === 'abstract_class_declaration') {
            ...
            const nameNode = sibling.childForFieldName?.('name');
            if (nameNode) {
              return property_symbol(nameNode.text as SymbolName, extract_location(nameNode));
            }
          }
          break;
        }
...
export const TYPESCRIPT_BUILDER_CONFIG: LanguageBuilderConfig = new Map([
  ...
  ["def.interface.method", {
    process: (capture: RawCapture, builder: DefinitionBuilder, context: ProcessingContext) => {
      ...
      const method_id = create_method_signature_id(capture, extract_symbol_name(capture));
  ...
  ["def.type_alias", {
    process: (capture: RawCapture, builder: DefinitionBuilder, context: ProcessingContext) => {
      ...
      builder.add_type({
        ...
        type_parameters: capture.node.parent ? extract_type_parameters(capture.node.parent) : []
      });
    }
  }],
