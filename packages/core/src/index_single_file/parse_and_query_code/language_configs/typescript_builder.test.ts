...
describe("TypeScript Builder Configuration", () => {
  ...
  function createRawCapture(name: string, node: SyntaxNode, text?: string): RawCapture {
    return {
      name,
      node,
      text: text || node.text,
  ...
  describe("Decorator handling", () => {
    ...
    it("should process method decorators", () => {
      ...
      const decoratorNode = classBodyNode?.children?.find(c => c.type === "decorator");
    ...
    it("should process property decorators", () => {
      ...
      const decoratorNode = fieldNode?.children?.find(c => c.type === "decorator");
