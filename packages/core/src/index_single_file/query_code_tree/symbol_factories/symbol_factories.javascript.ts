...
/** The node types that own a body, so an ancestor walk that reaches one has left the argument position. */
const CALLABLE_BOUNDARY_TYPES = new Set([
  "function",
  "function_expression",
 ...
 *
 * A callable is an argument of the call it is written directly inside, so the
 * walk stops at the first enclosing callable: a function merely written in
 * another callback's body — `register(function () { app.m = function () {} })` —
 * is not itself passed to `register`, and calling it one would put an edge in
 * the graph for a call nothing makes.
 */
export function detect_callback_context(
  ...
  while (current && depth < MAX_DEPTH) {
    if (CALLABLE_BOUNDARY_TYPES.has(current.type)) {
      break;
    }
