...
function analyze_ownership_context(
  ...
  if (
    !index.references ||
    !index.references.all_references ||
    !Array.isArray(index.references.all_references)
  ) {
    return null;
  }
 ...
 */
function find_borrowed_value_location(
  ownership: OwnershipContext
): Location | null {
...
export function should_use_ownership_semantics(
  ...
  try {
    const has_rust_captures = index.references.all_references.some(
      (ref) =>
 ...
 */
export function get_smart_pointer_methods(
