...
import type { TypeResolutionMap, FunctionResolutionMap } from "../types";
import type {
  MethodResolutionMap,
  MethodLookupContext,
...
export function resolve_pattern_aware_method_calls(
  ...
  for (const [file_path, index] of indices) {
    ...
    for (const [call_key, call_info] of conditional_calls) {
      ...
      if (enhanced_resolution) {
        ...
        const existing_calls =
          calls_to_method.get(enhanced_resolution.resolved_method) || [];
        ...
        if (
          ...
          calls_to_method.set(
            enhanced_resolution.resolved_method,
            existing_calls
          );
        }
    ...
    const async_resolutions = resolve_await_expressions(
      index,
      types,
      file_path
    );
    for (const [await_key, await_resolution] of async_resolutions) {
      ...
      const existing_calls =
        calls_to_method.get(await_resolution.resolved_method) || [];
  ...
  return {
    ...
    calls_to_method: calls_to_method as ReadonlyMap<
      SymbolId,
      readonly Location[]
    >,
...
function enhance_method_call_resolution(
  ...
  return {
    ...
    receiver_symbol: find_pattern_receiver_symbol(
      method_call,
      pattern_info,
      index
    ),
...
function resolve_pattern_receiver_type(
  ...
  for (const var_id of pattern_info.bound_variables) {
    ...
    if (var_type) {
      ...
      const symbol_at_receiver = find_symbol_at_location(
        index,
        method_call.receiver_location
      );
...
function find_pattern_receiver_symbol(
  ...
  return (
    find_symbol_at_location(index, method_call.receiver_location) ?? undefined
  );
}
...
function appears_to_be_rust_code(index: SemanticIndex): boolean {
  return (
    index.language === "rust" ||
    (index.references?.all_references?.some(
      (ref) =>
...
function find_symbol_at_location(
  ...
  if (index.references?.all_references) {
    for (const ref of index.references.all_references) {
      if (
        ref.location &&
        locations_match(ref.location, location) &&
        ref.symbol_id
      ) {
...
function resolve_await_expressions(
  ...
  for (const ref of index.references.all_references) {
    if (ref.modifiers?.is_await && ref.location) {
      const await_resolution = resolve_single_await_expression(
        ref,
        types,
        index
      );
...
function resolve_single_await_expression(
  ...
  const await_method_symbol = create_await_method_symbol(
    future_type,
    output_type
  );
 ...
 */
function create_await_method_symbol(
  future_type: TypeId,
  output_type: TypeId
): SymbolId {
