...
describe("Type Member Resolution Integration", () => {
  describe("Complete inheritance and member resolution pipeline", () => {
    it("should resolve complex inheritance hierarchy with member resolution", () => {
      ...
      const drawable_members = new Map<SymbolName, LocalMemberInfo>([
        [
          "draw" as SymbolName,
          createLocalMemberInfo("draw" as SymbolName, "method"),
        ],
        [
          "getColor" as SymbolName,
          createLocalMemberInfo("getColor" as SymbolName, "method"),
        ],
      ]);
      ...
      const comparable_members = new Map<SymbolName, LocalMemberInfo>([
        [
          "compareTo" as SymbolName,
          createLocalMemberInfo("compareTo" as SymbolName, "method"),
        ],
      ]);
      ...
      const shape_members = new Map<SymbolName, LocalMemberInfo>([
        [
          "area" as SymbolName,
          createLocalMemberInfo("area" as SymbolName, "method"),
        ],
        [
          "perimeter" as SymbolName,
          createLocalMemberInfo("perimeter" as SymbolName, "method"),
        ],
        [
          "name" as SymbolName,
          createLocalMemberInfo("name" as SymbolName, "property"),
        ],
        [
          "getId" as SymbolName,
          createLocalMemberInfo("getId" as SymbolName, "method", true),
        ], // static method
      ]);
      ...
      const circle_members = new Map<SymbolName, LocalMemberInfo>([
        [
          "radius" as SymbolName,
          createLocalMemberInfo("radius" as SymbolName, "property"),
        ],
        [
          "area" as SymbolName,
          createLocalMemberInfo("area" as SymbolName, "method"),
      ...
      const drawable_def = createLocalTypeDefinition(
        "IDrawable" as SymbolName,
        "interface",
        "interfaces.ts" as FilePath,
        drawable_members
      );
      const comparable_def = createLocalTypeDefinition(
        "IComparable" as SymbolName,
        "interface",
        "interfaces.ts" as FilePath,
        comparable_members
      );
      const shape_def = createLocalTypeDefinition(
        "Shape" as SymbolName,
        "class",
        "shapes.ts" as FilePath,
        shape_members,
        undefined,
        ["IDrawable" as SymbolName]
      );
      const circle_def = createLocalTypeDefinition(
        "Circle" as SymbolName,
        "class",
        "circle.ts" as FilePath,
        circle_members,
        ["Shape" as SymbolName],
        ["IComparable" as SymbolName]
      );
      ...
      const type_definitions = new Map<FilePath, LocalTypeDefinition[]>([
        [
          "test.ts" as FilePath,
          [drawable_def, comparable_def, shape_def, circle_def],
        ],
      ]);

      const resolved_imports = createReadonlyMap<
        FilePath,
        ReadonlyMap<SymbolName, SymbolId>
      >();

      // Step 1: Resolve inheritance hierarchy
      const inheritance_result = resolve_inheritance(
        type_definitions,
        resolved_imports
      );
      ...
      for (const member_name of expected_members) {
        expect(circle_result.all_members.has(member_name as SymbolName)).toBe(
          true
        );
      }
      ...
      expect(
        circle_result.all_members.get("radius" as SymbolName)!.inherited_from
      ).toBeUndefined(); // Direct
      expect(
        circle_result.all_members.get("area" as SymbolName)!.inherited_from
      ).toBeUndefined(); // Overridden
      expect(
        circle_result.all_members.get("perimeter" as SymbolName)!.inherited_from
      ).toBe(shape_id); // Inherited
      expect(
        circle_result.all_members.get("getId" as SymbolName)!.is_static
      ).toBe(true); // Static method preserved
    });
    ...
    it("should handle cross-file inheritance with member resolution", () => {
      ...
      const base_members = new Map<SymbolName, LocalMemberInfo>([
        [
          "connect" as SymbolName,
          createLocalMemberInfo("connect" as SymbolName, "method"),
        ],
        [
          "disconnect" as SymbolName,
          createLocalMemberInfo("disconnect" as SymbolName, "method"),
        ],
        [
          "logger" as SymbolName,
          createLocalMemberInfo("logger" as SymbolName, "property"),
        ],
      ]);

      const derived_members = new Map<SymbolName, LocalMemberInfo>([
        [
          "getUser" as SymbolName,
          createLocalMemberInfo("getUser" as SymbolName, "method"),
        ],
        [
          "updateUser" as SymbolName,
          createLocalMemberInfo("updateUser" as SymbolName, "method"),
        ],
        [
          "connect" as SymbolName,
          createLocalMemberInfo("connect" as SymbolName, "method"),
      ...
      const base_def = createLocalTypeDefinition(
        "BaseService" as SymbolName,
        "class",
        "base.ts" as FilePath,
        base_members
      );
      const derived_def = createLocalTypeDefinition(
      ...
      // Check overriding
      const connect_method = derived_result.all_members.get(
        "connect" as SymbolName
      );
      expect(connect_method!.inherited_from).toBeUndefined(); // Overridden, not inherited

      // Check inherited members
      const disconnect_method = derived_result.all_members.get(
        "disconnect" as SymbolName
      );
  ...
  describe("Error handling in integrated pipeline", () => {
    it("should handle broken inheritance chains gracefully", () => {
      ...
      const child_def = createLocalTypeDefinition(
        ...
        new Map([
          [
            ...
            createLocalMemberInfo("childMethod" as SymbolName, "method"),
          ],
        ]),
      ...
      expect(child_result.all_members.has("childMethod" as SymbolName)).toBe(
        true
      );
    });
    ...
    it("should handle complex circular inheritance", () => {
      ...
      const a_def = createLocalTypeDefinition(
        "A" as SymbolName,
        "class",
        "test.ts" as FilePath,
        new Map([
          [
            ...
            createLocalMemberInfo("methodA" as SymbolName, "method"),
          ],
        ]),
        ["B" as SymbolName]
      );

      const b_def = createLocalTypeDefinition(
        "B" as SymbolName,
        "class",
        "test.ts" as FilePath,
        new Map([
          [
            ...
            createLocalMemberInfo("methodB" as SymbolName, "method"),
          ],
        ]),
        ["C" as SymbolName]
      );

      const c_def = createLocalTypeDefinition(
        "C" as SymbolName,
        "class",
        "test.ts" as FilePath,
        new Map([
          [
            "methodC" as SymbolName,
            createLocalMemberInfo("methodC" as SymbolName, "method"),
          ],
        ]),
        ["A" as SymbolName]
      );
    ...
    it("should handle large realistic inheritance hierarchies efficiently", () => {
      ...
      for (const iface of interfaces) {
        const def = createLocalTypeDefinition(
          ...
          new Map([
            [
              `${iface.toLowerCase()}Method` as SymbolName,
              createLocalMemberInfo(
                `${iface.toLowerCase()}Method` as SymbolName,
                "method"
              ),
            ],
      ...
      for (const cls of hierarchies) {
        ...
        for (let i = 0; i < 10; i++) {
          const method_name =
            `${cls.name.toLowerCase()}Method${i}` as SymbolName;
          members.set(
            method_name,
            createLocalMemberInfo(method_name, "method")
          );
        }
    ...
    it("should handle multiple interfaces with conflicting method signatures", () => {
      ...
      const writer_members = new Map<SymbolName, LocalMemberInfo>([
        [
          ...
          createLocalMemberInfo("process" as SymbolName, "method"),
        ],
        [
          "write" as SymbolName,
          createLocalMemberInfo("write" as SymbolName, "method"),
        ],
      ]);
      ...
      const reader_members = new Map<SymbolName, LocalMemberInfo>([
        ...
        [
          "read" as SymbolName,
          createLocalMemberInfo("read" as SymbolName, "method"),
        ],
      ]);
      ...
      const writer_def = createLocalTypeDefinition(
        "IWriter" as SymbolName,
        "interface",
        "test.ts" as FilePath,
        writer_members
      );
      const reader_def = createLocalTypeDefinition(
        "IReader" as SymbolName,
        "interface",
        "test.ts" as FilePath,
        reader_members
      );
      const class_def = createLocalTypeDefinition(
        "FileHandler" as SymbolName,
        "class",
        "test.ts" as FilePath,
        class_members,
        undefined,
        ["IWriter" as SymbolName, "IReader" as SymbolName]
      );
      ...
      const class_result = resolve_type_members(
        class_id,
        class_def,
        hierarchy_map,
        all_type_definitions
      ...
      const process_method = class_result.all_members.get(
        "process" as SymbolName
      );
