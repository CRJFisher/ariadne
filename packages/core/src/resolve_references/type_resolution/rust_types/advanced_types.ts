...
export function resolve_unsafe_contexts(
  ...
  const unsafe_contexts = new Map<
    LocationKey,
    { is_unsafe: boolean; unsafe_scope_kind: string }
  >();
...
export function resolve_loop_constructs(
  ...
  const loop_constructs = new Map<
    LocationKey,
    { loop_type: string; iterator_type?: TypeId }
  >();
  ...
  for (const [scope_id, scope] of index.scopes) {
    if (scope.modifiers?.is_loop && scope.location) {
      ...
      if (loop_type === "for") {
        ...
        iterator_type = resolve_for_loop_iterator_type(
          scope,
          index,
          type_resolution
        );
