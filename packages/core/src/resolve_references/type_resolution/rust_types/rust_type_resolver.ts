...
import type { LocationKey, TypeId, FilePath } from "@ariadnejs/types";
...
export function resolve_all_rust_types(
  ...
  enhanced_resolution = integrate_pattern_matching_into_type_resolution(
    index,
    enhanced_resolution
  );

  // Resolve all type categories
  const reference_types = resolve_rust_reference_types(
    index,
    enhanced_resolution,
    file_path
  );
  const async_types = resolve_rust_async_types(
    index,
    enhanced_resolution,
    file_path
  );
  const function_types = resolve_rust_function_types(
    index,
    enhanced_resolution,
    file_path
  );
