...
import { TypeInfo } from "../../index_single_file/references/type_tracking/type_info";
...
describe("Type Registry Interfaces", () => {
  ...
  describe("Type Reassignment Functions", () => {
    ...
    describe("create_widening_reassignment", () => {
      it("should create a widening type reassignment", () => {
        const reassignment = create_widening_reassignment(
          fromType,
          toType,
          mockLocation
        );
    ...
    describe("create_neutral_reassignment", () => {
      it("should create a neutral type reassignment", () => {
        const reassignment = create_neutral_reassignment(
          fromType,
          toType,
          mockLocation
        );
    ...
    describe("validate_reassignment", () => {
      it("should validate correct reassignments", () => {
        const narrowing = create_narrowing_reassignment(
          fromType,
          toType,
          mockLocation
        );
        const widening = create_widening_reassignment(
          fromType,
          toType,
          mockLocation
        );
        const neutral = create_neutral_reassignment(
          fromType,
          toType,
          mockLocation
        );
  ...
  describe("Interface Structures", () => {
    ...
    describe("InheritanceInfo", () => {
      ...
      it("should represent combined inheritance", () => {
        ...
        expect(
          inheritance.all_members.get("inheritedMethod" as SymbolName)
        ).toBe(methodMember);
      });
    });
  ...
  describe("Complex Registry Scenarios", () => {
    it("should create a fully populated type context", () => {
      ...
      expect(context.registry.symbol_to_type.get(mockSymbolId)).toBe(
        mockTypeId
      );
      expect(context.registry.name_to_type.get(mockSymbolName)).toBe(
        mockTypeId
      );
      ...
      expect(context.variables.variable_types.get(mockLocation)).toBe(
        mockTypeId
      );
      expect(context.variables.reassignments.get(mockLocation)).toBe(
        reassignment
      );
      ...
      const unionType = create_union_type([
        mockTypeId,
        "TypeId:Other" as TypeId,
      ]);
      composite_types.set("TypeId:Union" as TypeId, unionType);

      expect(context.composite_types.get("TypeId:Union" as TypeId)).toBe(
        unionType
      );
    });

    it("should handle readonly properties correctly", () => {
      const { registry, symbol_to_type, defined_types } =
        create_test_registry(mockFilePath);
