...
import type { TypeInfo } from "../../index_single_file/references/type_tracking/type_info";
...
export interface TypeMemberMap {
  ...
  readonly instance_members: ReadonlyMap<
    TypeId,
    ReadonlyMap<SymbolName, MemberInfo>
  >;

  /** Static members by type */
  readonly static_members: ReadonlyMap<
    TypeId,
    ReadonlyMap<SymbolName, MemberInfo>
  >;
...
export interface VariableTypeMap {
  ...
  readonly scope_variables: ReadonlyMap<
    ScopeId,
    ReadonlyMap<SymbolName, TypeId>
  >;
}
...
export function create_intersection_type(
...
export function create_test_context(file_path: FilePath): {
  ...
  const instance_members = new Map<
    TypeId,
    ReadonlyMap<SymbolName, MemberInfo>
  >();
  ...
  const members: TypeMemberMap = {
    instance_members: instance_members as ReadonlyMap<
      TypeId,
      ReadonlyMap<SymbolName, MemberInfo>
    >,
    static_members: static_members as ReadonlyMap<
      TypeId,
      ReadonlyMap<SymbolName, MemberInfo>
    >,
  ...
  const variables: VariableTypeMap = {
    variable_type_info: variable_type_info as ReadonlyMap<
      Location,
      VariableTypeInfo
    >,
    ...
    scope_variables: scope_variables as ReadonlyMap<
      ScopeId,
      ReadonlyMap<SymbolName, TypeId>
    >,
  };
