...
import {
  ...
  type EnhancedMethodResolutionContext,
} from "./enhanced_context";
import {
  resolve_method_enhanced,
  type MethodIndex,
} from "./enhanced_heuristic_resolver";
...
export function resolve_methods_enhanced(
  ...
  for (const [file_path, index] of indices) {
    ...
    const enhanced_context = build_enhanced_context(
      index,
      file_imports,
      file_path
    );
...
export function compare_resolution_accuracy(
  ...
  console.log(
    `  Improvement: ${
      enhanced_methods - original_methods
  ...
  for (const [loc_key, enhanced_id] of enhanced_results.method_calls) {
    ...
    if (original_id) {
      if (original_id === enhanced_id) {
        ...
        conflicts++;
        console.warn(
          `Conflict at ${loc_key}: original=${original_id}, enhanced=${enhanced_id}`
        );
      }
    } else {
      new_resolutions++;
