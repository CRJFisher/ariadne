...
describe("the heap an arm is given", () => {
  ...
  it("admits microsoft/TypeScript on a 32 GB box, which is the corpus the previous fit refused", () => {
    const [refused] = RECORDED_FAILURE_TAXONOMY_BASELINE.not_measured;
    expect(refused.corpus).toEqual("microsoft/TypeScript");
    expect(heap_mb_for(refused.discovered_files)).toEqual(14240);
    expect(heap_mb_for(refused.discovered_files)).toBeLessThan(32768);
  });
});
