...
declare global {
  namespace Reflect {
    function getMetadata(key: string, target: any, propertyName?: string | symbol): any;
    function defineMetadata(key: string, value: any, target: any, propertyName?: string | symbol): void;
  }
}
...
(globalThis as any).Reflect = {
  ...Reflect,
  getMetadata: (key: string, target: any, propertyName?: string | symbol) => undefined,
  defineMetadata: (key: string, value: any, target: any, propertyName?: string | symbol) => {},
};
...
export function Log(target: any, propertyName: string, descriptor: PropertyDescriptor) {
  ...
  descriptor.value = function (...args: any[]) {
    console.log(`[${new Date().toISOString()}] Calling ${propertyName} with args:`, args);
    const result = originalMethod.apply(this, args);
    console.log(`[${new Date().toISOString()}] ${propertyName} returned:`, result);
    return result;
  };
...
export function Benchmark(target: any, propertyName: string, descriptor: PropertyDescriptor) {
  const originalMethod = descriptor.value;
  ...
  return function (target: any, propertyName: string) {
    ...
    const setter = (newValue: any) => {
      if (typeof newValue === 'string' && newValue.length < min) {
        throw new Error(`Property ${propertyName} must be at least ${min} characters long`);
      }
      value = newValue;
    };
...
export function ReadOnly(target: any, propertyName: string) {
  ...
  const setter = (newValue: any) => {
    if (hasBeenSet) {
      throw new Error(`Property ${propertyName} is read-only and cannot be modified`);
    }
    hasBeenSet = true;
  };
...
// Factory decorators
export function Column(options: { name?: string; type?: string; nullable?: boolean } = {}) {
  return function (target: any, propertyName: string) {
...
export class User {
  ...
  constructor(
    ...
    @Validate((value) => typeof value === 'string' && value.length > 0, 'Name is required') name: string,
    email: string
  ) {
    this.id = id;
    this.name = name;
...
export function Middleware(middleware: Function) {
  return function (target: any, propertyName: string, descriptor: PropertyDescriptor) {
    const middlewares = Reflect.getMetadata('middlewares', target, propertyName) || [];
...
export function Auditable<T extends { new (...args: any[]): {} }>(constructor: T) {
  return class extends constructor {
    auditLog: string[] = [];
