...
describe('Type Registry Index Exports', () => {
  ...
  it('should allow importing all functions and test them', async () => {
    ...
    const registry = create_empty_registry('/test.ts' as any);
    const memberMap = create_empty_member_map();
    const variableMap = create_empty_variable_map();
    const context = create_type_context('/test.ts' as any);
    ...
    const mockLocation = { file_path: '/test.ts' as any, line: 1, column: 1, end_line: 1, end_column: 1 };
    const narrowing = create_narrowing_reassignment('from' as any, 'to' as any, mockLocation);
    const widening = create_widening_reassignment('from' as any, 'to' as any, mockLocation);
    const neutral = create_neutral_reassignment('from' as any, 'to' as any, mockLocation);
    ...
    const unionType = create_union_type(['string', 'number'] as any);
    const intersectionType = create_intersection_type(['A', 'B'] as any);
    const arrayType = create_array_type('string' as any);
    const tupleType = create_tuple_type(['string', 'number'] as any);
  ...
  it('should have consistent exports between index and type_registry', async () => {
    const indexExports = await import('./index') as any;
    const typeRegistryExports = await import('./type_registry') as any;

    // Get all function exports
    const indexFunctions = Object.keys(indexExports).filter(key =>
      typeof indexExports[key] === 'function'
    );
    const typeRegistryFunctions = Object.keys(typeRegistryExports).filter(key =>
      typeof typeRegistryExports[key] === 'function'
    );
