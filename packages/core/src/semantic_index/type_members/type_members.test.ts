...
describe("Type Members", () => {
  ...
  describe("collect_type_members", () => {
    describe("Success Cases", () => {
      it("should collect members from class symbols", () => {
        ...
        const propertySymbol: SymbolDefinition = {
        ...
        const classMembers = result.instance_members.get("class_MyClass_type" as TypeId);
      ...
      it("should collect static members separately", () => {
        ...
        const staticMethodSymbol: SymbolDefinition = {
        ...
        const staticPropertySymbol: SymbolDefinition = {
        ...
        mockSymbols.set("static_property_symbol" as SymbolId, staticPropertySymbol);

        const result = collect_type_members(mockSymbols, mockScopes);

        const staticMembers = result.static_members.get("class_MyClass_type" as TypeId);
        ...
        const staticMethodInfo = staticMembers?.get("staticMethod" as SymbolName);
        expect(staticMethodInfo?.member_type).toBe("method");
        expect(staticMethodInfo?.is_static).toBe(true);

        const staticPropertyInfo = staticMembers?.get("STATIC_CONSTANT" as SymbolName);
      ...
      it("should collect constructor information", () => {
        ...
        const constructorInfo = result.constructors.get("class_MyClass_type" as TypeId);
      ...
      it("should collect method parameters", () => {
        ...
        const param1Symbol: SymbolDefinition = {
        ...
        const param2Symbol: SymbolDefinition = {
      ...
      it("should handle inheritance relationships", () => {
        ...
        const baseMethodSymbol: SymbolDefinition = {
        ...
        const derivedMethodSymbol: SymbolDefinition = {
        ...
        const baseInheritance = result.inheritance.get("class_BaseClass_type" as TypeId);
        const derivedInheritance = result.inheritance.get("class_DerivedClass_type" as TypeId);

        expect(baseInheritance?.extends_type).toBeUndefined();
        expect(derivedInheritance?.extends_type).toBe("class_BaseClass_type");

        expect(derivedInheritance?.all_ancestors).toContain("class_BaseClass_type" as TypeId);

        // Check that derived class inherits base methods
        expect(derivedInheritance?.all_members.has("baseMethod" as SymbolName)).toBe(true);
        expect(derivedInheritance?.all_members.has("derivedMethod" as SymbolName)).toBe(true);
      });
      ...
      it("should handle interface implementation", () => {
        ...
        const classInheritance = result.inheritance.get("class_MyClass_type" as TypeId);
        expect(classInheritance?.implements_types).toContain("interface_IInterface_type" as TypeId);
        expect(classInheritance?.all_ancestors).toContain("interface_IInterface_type" as TypeId);
      });
    ...
    describe("Edge Cases", () => {
      ...
      it("should handle circular inheritance", () => {
        const class1Symbol: SymbolDefinition = {
        ...
        const class2Symbol: SymbolDefinition = {
      ...
      it("should handle members without valid member_of", () => {
        const orphanMethodSymbol: SymbolDefinition = {
      ...
      it("should handle unknown parent classes", () => {
        ...
        const inheritance = result.inheritance.get("class_MyClass_type" as TypeId);
  ...
  describe("find_type_methods", () => {
    ...
    describe("Success Cases", () => {
      ...
      it("should include static methods when requested", () => {
        const result = find_type_methods("test_type" as TypeId, mockMembers, true);
      ...
      it("should handle types without methods", () => {
        const result = find_type_methods("non_existent_type" as TypeId, mockMembers);
    ...
    describe("Edge Cases", () => {
      ...
      it("should handle types without inheritance info", () => {
        ...
        const result = find_type_methods("test_type" as TypeId, membersWithoutInheritance);
  ...
  describe("resolve_method_on_type", () => {
    ...
    describe("Success Cases", () => {
      ...
      it("should prioritize own methods over inherited", () => {
        ...
        const instanceMembers = mockMembers.instance_members.get("test_type" as TypeId)!;
  ...
  describe("Integration Tests", () => {
    it("should handle complete type member collection and resolution", () => {
      ...
      const overriddenMethodSymbol: SymbolDefinition = {
      ...
      mockSymbols.set("base_method_symbol" as SymbolId, baseMethodSymbol);
      mockSymbols.set("derived_method_symbol" as SymbolId, derivedMethodSymbol);
      mockSymbols.set("overridden_method_symbol" as SymbolId, overriddenMethodSymbol);

      // Collect members
      const members = collect_type_members(mockSymbols, mockScopes);

      // Test inheritance resolution
      const derivedInheritance = members.inheritance.get("class_DerivedClass_type" as TypeId);
      expect(derivedInheritance?.extends_type).toBe("class_BaseClass_type");

      // Test method finding
      const derivedMethods = find_type_methods("class_DerivedClass_type" as TypeId, members);
