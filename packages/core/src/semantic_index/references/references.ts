...
} from "./type_flow_references/type_flow_references";
import {
  process_return_references,
  ReturnReference,
} from "./return_references/return_references";
import {
  MemberAccessReference,
  process_member_access_references,
} from "./member_access_references/member_access_references";
import {
  process_type_annotation_references,
  TypeAnnotationReference,
} from "./type_annotation_references/type_annotation_references";
...
export function process_references(
  ...
  let type_flows: LocalTypeFlowData = {
    constructor_calls: [],
    assignments: [],
    returns: [],
    call_assignments: [],
  ...
  const call_captures = ref_captures.filter(
    (c) =>
  ...
  if (all_captures.length > 0) {
    type_flows = extract_type_flow(all_captures, scopes, file_path);
  }
  ...
  const member_captures = ref_captures.filter(
    (c) =>
