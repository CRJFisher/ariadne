...
describe("Semantic Index Query Bug Fixes", () => {
  ...
  describe("query_tree_and_parse_captures", () => {
    it("should throw descriptive error when parser is not found for language", async () => {
      ...
      const { query_tree_and_parse_captures } = await import("./semantic_index");
      const { load_query } = await import("./query_loader");
    ...
    it("should handle undefined parser gracefully", async () => {
      const { query_tree_and_parse_captures } = await import("./semantic_index");
      const { load_query, LANGUAGE_TO_TREESITTER_LANG } = await import("./query_loader");
  ...
  describe("Edge Cases", () => {
    it("should handle null tree gracefully", async () => {
      ...
      expect(() => {
        query_tree_and_parse_captures('javascript', null as any, file_path);
