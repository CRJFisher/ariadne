...
export interface CallChainNode {
  caller: string; // Function/method making the call
  callee: string; // Function/method being called
  location: Point;
  file_path: string;
  call_type: 'function' | 'method' | 'constructor';
  depth: number; // Depth in the call chain
}
...
export interface CallChain {
  root: string; // Starting function
  nodes: CallChainNode[];
  is_recursive: boolean;
  max_depth: number;
  cycle_point?: string; // Where recursion occurs
}
...
export interface CallChainAnalysisResult {
  ...
  call_graph: Map<string, Set<string>>; // Adjacency list representation
}
...
export function build_call_chains(
  ...
  const all_callees = new Set<string>();
  ...
  const roots = Array.from(all_callers).filter(caller => !all_callees.has(caller));
...
function build_call_graph(
  ...
  for (const call of calls) {
    ...
    if ('caller_def' in call && 'called_def' in call) {
      ...
      // ConstructorCallInfo
      caller = '<module>'; // Constructor calls might not have explicit caller
 ...
 */
export function detect_recursion(
  chains: CallChain[]
): CallChain[] {
