...
describe('Constructor Call Detection', () => {
  ...
  describe('JavaScript', () => {
    ...
    it('should detect factory functions', () => {
      ...
      const componentCall = calls.find(c => c.constructor_name === 'Component');
    ...
    it('should track type assignments', () => {
      const source = `
        const obj1 = new MyClass();
        let obj2 = new AnotherClass();
      `;
      ...
      const assignments = get_type_assignments(context);
      
      expect(assignments).toHaveLength(2);
      expect(assignments[0].variable_name).toBe('obj1');
      expect(assignments[0].type_name).toBe('MyClass');
      expect(assignments[1].variable_name).toBe('obj2');
      expect(assignments[1].type_name).toBe('AnotherClass');
    });
  });
  ...
  describe('TypeScript', () => {
    ...
    it('should detect generic constructor calls', () => {
      ...
      const containerCall = calls.find(c => c.constructor_name === 'Container');
...
class MyClass:
    def __init__(self):
      ...
      const dataClassCall = calls.find(c => c.constructor_name === 'DataClass');
      const processorCall = calls.find(c => c.constructor_name === 'Processor');
