...
export function enrich_method_calls(
  ...
  // TODO: fix to use immutable update pattern
  ...
  return hierarchy_enriched.map((call) => {
    const enriched: MethodCall = { ...call };
...
export function enrich_constructor_calls(
  ...
  return type_enriched.map((call) => {
    const enriched: ConstructorCall = { ...call };
...
export function resolve_polymorphic_targets(
  call: any,
...
export function calculate_dispatch_probability(
  call: MethodCall,
  target_class: any
): number {
...
export function calculate_method_confidence(
  call: MethodCall,
  hierarchy: ClassHierarchy
): number {
  let score = 0.0;
...
export function enrich_function_calls(
  ...
  return calls.map((call) => {
    const enriched: FunctionCall = { ...call };
...
export function calculate_function_confidence(
  call: FunctionCall
): number {
  let score = 0.0;
