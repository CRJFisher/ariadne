...
export function extract_rust_class_relationships(
  ...
  const type_node = find_node_at_position(
  ...
  find_trait_implementations(info.definition.name, tree.rootNode, info, source_code);
...
function extract_derive_attribute(
  ...
  if (!name_node || name_node.type !== 'identifier' || name_node.text !== 'derive') {
    return traits;
  }
...
function extract_type_name(node: SyntaxNode, source_code: string): string | null {
  ...
  if (node.type === 'trait_bound') {
    ...
    const type_node = find_child_by_type(node, 'type_identifier') ||
 ...
 */
export function is_generic_type(
  def_node: SyntaxNode
): boolean {
...
export function extract_generic_parameters(
  ...
  for (let i = 0; i < type_params.childCount; i++) {
    ...
    if (param.type === 'type_parameter' || param.type === 'lifetime_parameter') {
