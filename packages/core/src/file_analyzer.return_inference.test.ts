...
describe('Return Type Inference Integration', () => {
  ...
  it('should handle async function return types', async () => {
    ...
    console.log('Async functions:', analysis.functions.map(f => ({
      name: f.name,
      return_type: f.signature.return_type,
  ...
  it('should infer return types for arrow functions', async () => {
    ...
    console.log('Arrow functions:', analysis.functions.map(f => ({
      name: f.name,
      return_type: f.signature.return_type
    })));
  ...
  it('should handle TypeScript explicit return types', async () => {
    ...
    console.log('TypeScript functions:', analysis.functions.map(f => ({
      name: f.name,
      return_type: f.signature.return_type
    })));
...
def get_dict():
    ...
    console.log('Python functions:', analysis.functions.map(f => ({
      name: f.name,
      return_type: f.signature.return_type
    })));
...
fn get_result() -> Result<String, Error> {
    ...
    console.log('Rust functions:', analysis.functions.map(f => ({
      name: f.name,
      return_type: f.signature.return_type
    })));
...
class Calculator {
  ...
  async compute() {
    ...
    if (calc_class) {
      console.log('Method return types:', calc_class.methods.map(m => ({
        name: m.name,
