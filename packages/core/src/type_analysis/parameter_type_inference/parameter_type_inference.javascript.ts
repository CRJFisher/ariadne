...
function analyze_node_for_usage(
  ...
  if (
    ...
    if (
      object &&
      source_code.substring(object.startIndex, object.endIndex) === param_name
    ) {
      if (node.type === "subscript_expression") {
        ...
        if (property) {
          const prop_name = source_code.substring(
            property.startIndex,
            property.endIndex
          );
          ...
          if (
            ...
            ["charAt", "substring", "toLowerCase", "toUpperCase"].includes(
              prop_name
            )
          ) {
  ...
  if (node.type === "binary_expression") {
    ...
    if (operator) {
      ...
      if (["+", "-", "*", "/", "%", "**"].includes(op)) {
        ...
        if (
          (left &&
            source_code.substring(left.startIndex, left.endIndex) ===
              param_name) ||
          (right &&
            source_code.substring(right.startIndex, right.endIndex) ===
              param_name)
        ) {
