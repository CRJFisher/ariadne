...
describe('type_tracking', () => {
  describe('Core type tracking', () => {
    ...
    it('should track multiple type assignments at different positions', () => {
      ...
      const type_at_7 = get_variable_type(tracker, 'x', { row: 7, column: 0 });
      expect(type_at_7?.type_name).toBe('number');
      
      const type_at_12 = get_variable_type(tracker, 'x', { row: 12, column: 0 });
  ...
  describe('Local type tracking', () => {
    it('should create local tracker with parent fallback', () => {
      let parent = create_file_type_tracker();
      parent = set_variable_type(parent, 'parentVar', {
        type_name: 'number',
        type_kind: 'primitive',
      ...
      local = set_local_variable_type(local, 'localVar', {
        type_name: 'string',
        type_kind: 'primitive',
        position: { row: 5, column: 0 },
        confidence: 'explicit',
      ...
      const local_type = get_local_variable_type(local, 'localVar');
      expect(local_type?.type_name).toBe('string');
      
      // Should fall back to parent
      const parent_type = get_local_variable_type(local, 'parentVar');
      expect(parent_type?.type_name).toBe('number');
    });
  });

  describe('Project type registry', () => {
    it('should register and retrieve exported types', () => {
      let registry = create_project_type_registry();
      
      registry = register_export(registry, '/src/file.ts', 'MyClass', {
        className: 'MyClass',
        classDef: {
          name: 'MyClass',
          type: 'class',
      ...
      const type_info = get_imported_type(registry, '/src/file.ts', 'MyClass');
      expect(type_info).toBeDefined();
      expect(type_info?.className).toBe('MyClass');
    });
  });
  ...
  describe('JavaScript type tracking', () => {
    it('should track JavaScript variable assignments', () => {
      ...
      const tracker = process_file_for_types(code, tree.rootNode, context);
    ...
    it('should track ES6 imports', () => {
      ...
      expect(get_imported_class(tracker, 'Component')?.source_module).toBe('react');
      expect(get_imported_class(tracker, 'useState')?.source_module).toBe('react');
    });
  ...
  describe('TypeScript type tracking', () => {
    it('should track TypeScript type annotations', () => {
      ...
      expect(get_variable_type(tracker, 'tuple')?.type_name).toBe('(number, string)');
      expect(get_variable_type(tracker, 'union')?.type_name).toBe('string | number');
      expect(get_variable_type(tracker, 'generic')?.type_name).toBe('Array<string>');
    });
...
union_var: Union[str, int] = "test"
      ...
      expect(get_variable_type(tracker, 'list_var')?.type_name).toBe('List[str]');
      expect(get_variable_type(tracker, 'dict_var')?.type_name).toBe('Dict[str, int]');
      expect(get_variable_type(tracker, 'optional_var')?.type_name).toBe('Optional[str]');
      expect(get_variable_type(tracker, 'union_var')?.type_name).toBe('Union[str, int]');
    });
...
from package.module import SomeClass as Alias
      ...
      expect(get_imported_class(tracker, 'MyClass')?.source_module).toBe('mymodule');
      expect(get_imported_class(tracker, 'Alias')?.class_name).toBe('SomeClass');
    });
...
let tuple_var: (i32, String) = (42, "test".to_string());
      ...
      expect(get_variable_type(tracker, 'vec_var')?.type_name).toBe('Vec<String>');
      expect(get_variable_type(tracker, 'option_var')?.type_name).toBe('Option<i32>');
      expect(get_variable_type(tracker, 'result_var')?.type_name).toBe('Result<String, Error>');
      expect(get_variable_type(tracker, 'array_var')?.type_name).toBe('[i32; 5]');
      expect(get_variable_type(tracker, 'tuple_var')?.type_name).toBe('(i32, String)');
    });
...
use crate::utils::helper as h;
      ...
      expect(get_imported_class(tracker, 'HashMap')?.source_module).toBe('std::collections::HashMap');
      expect(get_imported_class(tracker, 'Read')?.source_module).toBe('std::io::Read');
      expect(get_imported_class(tracker, 'Write')?.source_module).toBe('std::io::Write');
      expect(get_imported_class(tracker, 'MyStruct')?.source_module).toBe('super::module::MyStruct');
...
let tuple_lit = (1, "test");
  ...
  describe('Type utilities', () => {
    ...
    it('should check type assignability', () => {
      ...
      expect(is_type_assignable(string_type, string_type, 'typescript')).toBe(true);
      
      // Any can be assigned to any type
      expect(is_type_assignable(any_type, string_type, 'typescript')).toBe(true);
      
      // Any type can be assigned to any
      expect(is_type_assignable(string_type, any_type, 'typescript')).toBe(true);
      
      // Different primitives are not assignable
      expect(is_type_assignable(string_type, number_type, 'typescript')).toBe(false);
    });
  });
});