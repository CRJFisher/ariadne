/**
 * Structural guard (TASK-190.36.4 item 6, the Z15 backstop): no builtin check may
 * key on `EnrichedEntryPoint.tree_size` or `.definition_features`.
 *
 ...
 * misfire on the fields the author never verified. This test walks every
 * `builtins/check_*.ts` and fails the build on any member access reading a
 * denylisted field — a closed denylist with low false-positive risk (no
 * production check reads either field).
 */
...
/** Every `check_*.ts` in this directory (the production builtins), excluding tests. */
function collect_check_sources(dir: string): string[] {
  return fs
    .readdirSync(dir, { withFileTypes: true })
    .filter((e) => e.isFile() && e.name.startsWith("check_") && e.name.endsWith(".ts") && !e.name.endsWith(".test.ts"))
