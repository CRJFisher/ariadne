...
import { create_memory_storage, MemoryStorage } from '../../storage/memory_storage';
...
describe('Graph Builder', () => {
  ...
  describe('File Analysis', () => {
    ...
    test('should analyze a TypeScript file with classes', async () => {
      ...
      const class_node = result.nodes.find(n => n.type === 'class' && n.name === 'Person');
    ...
    test('should detect imports and exports', async () => {
      ...
      const import_info = result.imports.find(i => i.module_path === './helper');
      expect(import_info).toBeDefined();
      
      // Should detect exports
      expect(result.exports.length).toBeGreaterThan(0);
      const export_info = result.exports.find(e => e.exported_name === 'process');
  ...
  describe('Incremental Updates', () => {
    test('should update graph when file changes', async () => {
      ...
      const updated_file = create_test_file('test.js', updated_code, 'javascript');
      await config.storage.update_file(updated_file);
      
      // Update graph incrementally
      const updated_graph = await update_file_graph('test.js', config, initial_graph);
    ...
    test('should handle file deletion', async () => {
      ...
      // Update graph
      const updated_graph = await update_file_graph('to-delete.js', config, graph);
  ...
  describe('Graph Queries', () => {
    ...
    test('should query by file path', async () => {
      const file1 = create_test_file('file1.js', 'function f1() {}', 'javascript');
      const file2 = create_test_file('file2.js', 'function f2() {}', 'javascript');
      ...
      const file1_nodes = file1_query.nodes.filter(n => n.file_path === 'file1.js');
      expect(file1_nodes.length).toBeGreaterThan(0);
      
      // Should not include nodes from file2
      const file2_nodes = file1_query.nodes.filter(n => n.file_path === 'file2.js');
    ...
    test('should query by name pattern', async () => {
      ...
      const process_func = handle_query.nodes.find(n => n.name === 'processData');
