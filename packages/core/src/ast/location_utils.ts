...
export function is_standard_location(location: AnyLocationFormat): location is StandardLocation {
  return 'line' in location && typeof location.line === 'number';
}

export function is_range_location(location: AnyLocationFormat): location is RangeLocation {
  return 'start' in location && typeof location.start === 'object';
}

export function is_row_location(location: AnyLocationFormat): location is RowLocation {
 ...
 */
export function extract_target_position(location: AnyLocationFormat): { row: number; column: number } {
  if (is_standard_location(location)) {
    ...
    // is_row_location(location) - Format: { row: number, column: number } - already 0-based
    return {
      row: location.row,
