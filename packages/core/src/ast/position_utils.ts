...
import { Point, SimpleRange } from '@ariadnejs/types';
 ...
 */
export function range_contains_point(range: SimpleRange, point: Point): boolean {
 ...
 */
export function range_contains_range(range1: SimpleRange, range2: SimpleRange): boolean {
  return range_contains_point(range1, range2.start) && 
 ...
 */
export function range_intersection(
  range1: SimpleRange,
  range2: SimpleRange
): SimpleRange | null {
  const start = compare_points(range1.start, range2.start) > 0 ? range1.start : range2.start;
  const end = compare_points(range1.end, range2.end) < 0 ? range1.end : range2.end;
 ...
 */
export function merge_ranges(ranges: SimpleRange[]): SimpleRange[] {
  if (ranges.length === 0) return [];
  
  // Sort ranges by start position
  const sorted = [...ranges].sort((a, b) => compare_points(a.start, b.start));
  
  const merged: SimpleRange[] = [sorted[0]];
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (compare_points(current.start, last.end) <= 0) {
      // Merge ranges
      last.end = compare_points(current.end, last.end) > 0 ? current.end : last.end;
    } else {
 ...
 */
export function create_range(start: Point, end: Point): SimpleRange {
 ...
 */
export function expand_range(range: SimpleRange, lines: number): SimpleRange {
  return {
    start: {
 ...
 */
export function range_size(range: SimpleRange): number {
