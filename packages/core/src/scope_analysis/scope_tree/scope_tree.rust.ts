...
export function build_rust_scope_tree(
  ...
  const tree = create_scope_tree('rust', file_path);
...
function traverse_rust_ast(
  ...
  if (node.type === 'impl_item') {
    ...
    if (type_node) {
      context = {
        ...context,
        current_impl_type: context.source_code.substring(type_node.startIndex, type_node.endIndex)
      };
    }
  }
...
function extract_rust_generics(
  ...
  for (let i = 0; i < type_params.childCount; i++) {
    ...
    if (param && (param.type === 'type_identifier' || param.type === 'lifetime')) {
...
function extract_rust_parameter_symbols(
  ...
  if (param_node.type === 'parameter') {
    ...
    symbols.push({
      ...
      type_info: source_code.substring(param_node.startIndex, param_node.endIndex)
    });
    ...
    for (let i = 0; i < param_node.childCount; i++) {
      ...
      if (child && child.type !== '|' && child.type !== ',') {
        const child_symbols = extract_rust_parameter_symbols(child, source_code);
  ...
  switch (pattern.type) {
      ...
      if (fields) {
        for (let i = 0; i < fields.childCount; i++) {
          ...
          if (field && field.type === 'field_pattern') {
            ...
            if (field_name) {
              names.push(source_code.substring(field_name.startIndex, field_name.endIndex));
            }
    ...
    case 'ref_pattern':
      ...
      if (ref_ident && ref_ident.type === 'identifier') {
        names.push(source_code.substring(ref_ident.startIndex, ref_ident.endIndex));
      }
      break;
...
function extract_rust_symbols(
  ...
  if (node.type === 'if_let_expression' || node.type === 'while_let_expression') {
    const pattern = node.childForFieldName('pattern');
    if (pattern) {
      const names = extract_pattern_bindings(pattern, source_code);
      for (const name of names) {
...
function extract_use_names(
  ...
  if (use_node.type === 'identifier') {
    ...
    if (name_part) {
      names.push(source_code.substring(name_part.startIndex, name_part.endIndex));
    }
...
function extract_rust_scope_metadata(
  ...
  for (let i = 0; i < node.childCount; i++) {
    ...
    if (child && child.type === 'visibility_modifier') {
      metadata.visibility = source_code.substring(child.startIndex, child.endIndex);
      break;
    }
  }
...
function is_rust_prelude_item(name: string): boolean {
  const prelude = [
    ...
    'From', 'Into', 'AsRef', 'AsMut', 'Drop', 'Fn', 'FnMut', 'FnOnce',
    'std', 'core', 'alloc', 'collections', 'fmt', 'io', 'fs',
