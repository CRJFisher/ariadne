...
export function extract_python_imports(
  ...
  const visit = (node: SyntaxNode) => {
    if (node.type === 'import_statement') {
      const import_info = extract_import_statement(node, source_code);
      if (import_info) imports.push(...import_info);
    } else if (node.type === 'import_from_statement') {
      const import_info = extract_from_import_statement(node, source_code);
      if (import_info) imports.push(...import_info);
    }
...
/**
 * Extract import statement (import module)
 */
function extract_import_statement(
  ...
  for (const child of import_node.children) {
    if (child.type === 'aliased_import') {
      ...
      if (name_node) {
        const module_path = source_code.substring(name_node.startIndex, name_node.endIndex);
        ...
        imports.push({
          name: alias,
          source_name: module_path !== alias ? module_path : undefined,
          module_path,
          range: {
      ...
      const module_path = source_code.substring(child.startIndex, child.endIndex);
      const name = module_path.split('.').pop()!;
      
      imports.push({
        name,
        source_name: module_path !== name ? module_path : undefined,
        module_path,
        range: {
          start: { row: import_node.startPosition.row, column: import_node.startPosition.column },
          end: { row: import_node.endPosition.row, column: import_node.endPosition.column }
 ...
 */
function extract_from_import_statement(
  import_node: SyntaxNode,
  source_code: string
): ImportInfo[] | undefined {
  const imports: ImportInfo[] = [];
  ...
  // Get the module path
  const module_node = import_node.childForFieldName('module_name');
  if (!module_node) return undefined;
  
  const module_path = source_code.substring(module_node.startIndex, module_node.endIndex);
  
  // Find imported names
  for (const child of import_node.children) {
    if (child.type === 'import_list' || child.type === 'aliased_import' || child.type === 'identifier' || child.type === 'dotted_name') {
      if (child.type === 'import_list') {
        // Multiple imports
        for (const item of child.children) {
          if (item.type === 'aliased_import') {
            const name_node = item.childForFieldName('name');
            const alias_node = item.childForFieldName('alias');
            ...
            const name = source_code.substring(item.startIndex, item.endIndex);
            imports.push({
              name,
              module_path,
              range: {
          ...
          imports.push({
            ...
            range: {
              start: { row: import_node.startPosition.row, column: import_node.startPosition.column },
                 ...
                 child.text !== 'from' && child.text !== 'import') {
        // Single unaliased import
        const name = source_code.substring(child.startIndex, child.endIndex);
        if (name !== module_path) {  // Not the module name itself
          imports.push({
            name,
            module_path,
            range: {
...
/**
 * Extract Python exports (__all__ list)
 */
export function extract_python_exports(
  ...
  const visit = (node: SyntaxNode) => {
    if (node.type === 'assignment') {
      ...
      if (left && left.type === 'identifier' && left.text === '__all__' && right) {
        // Extract list of exported names
        if (right.type === 'list') {
          for (const child of right.children) {
            if (child.type === 'string') {
              // Remove quotes
              const name = child.text.slice(1, -1);
              exports.push({
                name,
                range: {
