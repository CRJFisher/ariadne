...
export function resolve_function_call(
    ...
    if (file_definitions) {
      ...
      for (const [symbol_id, func_def] of functions) {
        if (func_def.name === call.callee) {
          ...
          if (!call_scope || is_function_accessible_from_scope(func_def, call_scope, call.location, context)) {
            return func_def;
          }
        }
      }
  ...
  function parse_qualified_name(callee: SymbolName): {
    ...
    const namespace = parts.slice(0, -1).join(callee_str.includes("::") ? "::" : ".") as SymbolName;
  ...
  function is_function_accessible_from_scope(
    ...
    const func_scope = find_scope_at_location(
      scope_tree,
      func_def.location
    );
