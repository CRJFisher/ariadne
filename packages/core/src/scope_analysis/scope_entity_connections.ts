...
function process_functions(
  ...
  for (const func of functions) {
    ...
    if (func_scope.parent_id) {
      const parent_contents = connections.scope_contents.get(func_scope.parent_id);
      if (parent_contents) {
...
function process_classes(
  ...
  for (const cls of classes) {
    ...
    if (class_scope.parent_id) {
      const parent_contents = connections.scope_contents.get(class_scope.parent_id);
      if (parent_contents) {
...
function process_variables(
  ...
  for (const variable of variables) {
    ...
    const containing_scope = find_scope_at_position(scope_tree, variable.location);
...
export function is_entity_visible_from_scope(
  ...
  const defining_scope_id = connections.entity_defining_scope.get(entity_symbol);
