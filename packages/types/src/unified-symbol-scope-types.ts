...
/**
 * Unified symbol definition that extends SemanticNode
 * Replaces SymbolDefinition, ScopeSymbol, and various Definition types
 */
export interface UnifiedSymbol extends SemanticNode {
  readonly id: SymbolId;
  readonly name: SymbolName;
  readonly kind: UnifiedSymbolKind;
 ...
 * Unified scope node
 */
export interface UnifiedScope extends SemanticNode {
  readonly path: ScopePath;                  // Full scope path
  readonly type: UnifiedScopeType;
...
/**
 * Unified symbol index for a file or project
 */
export interface UnifiedSymbolIndex {
  readonly symbols: ReadonlyMap<SymbolId, UnifiedSymbol>;
  readonly scopes: ReadonlyMap<ScopePath, UnifiedScope>;
...
function get_node_type_for_kind(kind: SymbolKind): string {
  switch (kind) {
    ...
    case "variable": return "variable_declaration";
    case "type": return "type_alias_declaration";
    case "enum": return "enum_declaration";
    case "namespace": return "namespace_declaration";
    case "parameter": return "parameter";
    case "property": return "property_definition";
    case "method": return "method_definition";
    case "import": return "import_statement";
    case "export": return "export_statement";
  }
}
...
export function is_symbol_accessible(
  ...
  if (symbol.visibility === "protected") {
    ...
    return from_scope.startsWith(symbol.scope_path.split('.').slice(0, -1).join('.'));
  }
