...
describe('Call Graph Integration Tests', () => {
  ...
  describe('TypeScript cross-file imports', () => {
    test('resolves imports across multiple files', () => {
      ...
      // Core utilities
      fs.writeFileSync(path.join(libDir, 'core.ts'), `
...
`);
      ...
      // Math utilities
      fs.writeFileSync(path.join(libDir, 'math.ts'), `
...
`);
      ...
      const addCallsLog = addNode!.calls.some(c => c.symbol === `${coreModule}#log`);
      ...
      const processCallsAdd = processNode!.calls.some(c => c.symbol === `${mathModule}#add`);
      const processCallsMultiply = processNode!.calls.some(c => c.symbol === `${mathModule}#multiply`);
      ...
      const processCallsError = processNode!.calls.some(c => c.symbol === `${coreModule}#error`);
    ...
    test('handles circular dependencies', () => {
      ...
      fs.writeFileSync(path.join(tempDir, 'moduleA.ts'), `
...
`);
      ...
      fs.writeFileSync(path.join(tempDir, 'moduleB.ts'), `
...
`);
      ...
      expect(funcANode!.calls.some(c => c.symbol === `${moduleB}#functionB`)).toBe(true);
      expect(funcBNode!.calls.some(c => c.symbol === `${moduleA}#helperA`)).toBe(true);
    });
  });
...
`);
  ...
  describe('Performance with large projects', () => {
    test('handles projects with many files efficiently', () => {
      ...
      for (let i = 0; i < 10; i++) {
        // Utility files
        fs.writeFileSync(path.join(utilsDir, `util${i}.ts`), `
...
`);
        ...
        fs.writeFileSync(path.join(servicesDir, `service${i}.ts`), `
...
`);
        ...
        fs.writeFileSync(path.join(componentsDir, `component${i}.ts`), `
...
${Array.from({ length: 10 }, (_, i) => `import { Component${i} } from './components/component${i}';`).join('\n')}
...
`);
  ...
  describe('Call graph filtering', () => {
    test('max_depth limits graph traversal', () => {
      ...
      fs.writeFileSync(path.join(tempDir, 'deep.ts'), `
...
`);
    ...
    test('file_filter excludes unwanted files', () => {
      ...
      fs.writeFileSync(path.join(tempDir, 'math.ts'), `
export function add(a: number, b: number) { return a + b; }
export function subtract(a: number, b: number) { return a - b; }
`);

      fs.writeFileSync(path.join(tempDir, 'math.test.ts'), `
...
`);
      ...
      const mainModule = normalize_module_path(path.join(tempDir, 'main.ts'));
      const testModule = normalize_module_path(path.join(tempDir, 'math.test.ts'));
