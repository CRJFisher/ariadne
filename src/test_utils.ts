...
import { ScopeGraph, Def, Ref, Import, Scope } from './graph';
...
export function debug_scope_graph(graph: ScopeGraph, source_code: string): ScopeDebug {
  ...
  const get_context = (range: any, name: string): string => {
    ...
    if (name_index >= 0) {
      const highlighted = line.substring(0, name_index) + 
                         '§' + name + '§' + 
  ...
  const build_scope_debug = (scope_id: number): ScopeDebug => {
    ...
    const scope_defs = defs.filter(def => {
      ...
      return edges.some(e => e.source_id === def.id && e.target_id === scope_id);
    });
    
    // Find imports in this scope
    const scope_imports = imports.filter(imp => {
      const edges = graph.getEdges('import_to_scope');
      return edges.some(e => e.source_id === imp.id && e.target_id === scope_id);
    });
    ...
    const def_debugs: DefDebug[] = scope_defs.map(def => {
      ...
      const ref_contexts = def_refs.map(ref => get_context(ref.range, ref.name));
    ...
    const import_debugs: ImportDebug[] = scope_imports.map(imp => {
      ...
      const import_refs = refs.filter(ref => {
        ...
        return edges.some(e => e.source_id === ref.id && e.target_id === imp.id);
      });
      const ref_contexts = import_refs.map(ref => get_context(ref.range, ref.name));
    ...
    const child_scopes = scopes.filter(child => {
      ...
      return edges.some(e => e.source_id === child.id && e.target_id === scope_id);
    });
    
    // Recursively build child scope debug info
    const child_debugs = child_scopes.map(child => build_scope_debug(child.id));
    ...
    const scope_refs = refs.filter(ref => {
      ...
      const has_target = def_edges.some(e => e.source_id === ref.id) || 
