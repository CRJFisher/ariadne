...
describe('normalize_module_path', () => {
  test('removes file extension', () => {
    expect(normalize_module_path('src/utils/helpers.ts')).toBe('src/utils/helpers');
    expect(normalize_module_path('src/utils/helpers.js')).toBe('src/utils/helpers');
    expect(normalize_module_path('src/utils/helpers.py')).toBe('src/utils/helpers');
    expect(normalize_module_path('src/utils/helpers.rs')).toBe('src/utils/helpers');
  });

  test('handles files with multiple dots', () => {
    expect(normalize_module_path('src/utils/helpers.test.ts')).toBe('src/utils/helpers.test');
    expect(normalize_module_path('src/config.prod.js')).toBe('src/config.prod');
  });

  test('normalizes backslashes to forward slashes', () => {
    expect(normalize_module_path('src\\utils\\helpers.ts')).toBe('src/utils/helpers');
    expect(normalize_module_path('src\\components\\Button.tsx')).toBe('src/components/Button');
  });

  test('removes leading slash', () => {
    expect(normalize_module_path('/src/utils/helpers.ts')).toBe('src/utils/helpers');
    expect(normalize_module_path('/absolute/path/file.js')).toBe('absolute/path/file');
  });

  test('handles files without extension', () => {
    expect(normalize_module_path('src/utils/Makefile')).toBe('src/utils/Makefile');
...
describe('get_symbol_id', () => {
  ...
  test('handles nested paths', () => {
    ...
    expect(get_symbol_id(def)).toBe('src/components/forms/inputs/TextInput#deepFunction');
  });
  ...
  test('handles anonymous functions with position', () => {
    ...
    expect(get_symbol_id(def)).toBe('src/utils/callbacks#<anonymous_line_42_col_15>');
  });
});
...
describe('parse_symbol_id', () => {
  ...
  test('throws on invalid format', () => {
    expect(() => parse_symbol_id('invalid_symbol_id')).toThrow('Invalid symbol ID format');
...
describe('create_method_symbol_id', () => {
  ...
  test('handles nested classes', () => {
    const id = create_method_symbol_id('src/ui/components', 'Button.Icon', 'render');
...
describe('get_symbol_container', () => {
  test('extracts container from method symbols', () => {
    ...
    expect(get_symbol_container('src/ui/Button#Component.render')).toBe('Component');
  });
  ...
  test('handles nested containers', () => {
    expect(get_symbol_container('src/ui/forms#Form.Field.validate')).toBe('Form');
  });
});

describe('get_unqualified_name', () => {
  test('returns unqualified name for methods', () => {
    expect(get_unqualified_name('src/models/User#User.validate')).toBe('validate');
    expect(get_unqualified_name('src/ui/Button#Button.render')).toBe('render');
  });

  test('returns full name for non-methods', () => {
    expect(get_unqualified_name('src/utils/helpers#process_data')).toBe('process_data');
    expect(get_unqualified_name('src/index#main')).toBe('main');
  });

  test('handles nested qualifications', () => {
    expect(get_unqualified_name('src/ui/forms#Form.Field.validate')).toBe('validate');
  });
});

describe('same_module', () => {
  test('identifies same modules with different extensions', () => {
    expect(same_module('src/utils/helpers.ts', 'src/utils/helpers.js')).toBe(true);
  ...
  test('handles .d.ts files correctly', () => {
    ...
    expect(normalize_module_path('src/models/User.d.ts')).toBe('src/models/User.d');
    expect(same_module('src/models/User.d.ts', 'src/models/User.d.ts')).toBe(true);
    // But User.ts and User.d.ts are different modules
    expect(same_module('src/models/User.ts', 'src/models/User.d.ts')).toBe(false);
  });

  test('identifies same modules with different path separators', () => {
    expect(same_module('src/utils/helpers.ts', 'src\\utils\\helpers.ts')).toBe(true);
    expect(same_module('src\\models\\User.ts', 'src/models/User.ts')).toBe(true);
  });

  test('identifies different modules', () => {
    expect(same_module('src/utils/helpers.ts', 'src/utils/strings.ts')).toBe(false);
    expect(same_module('src/models/User.ts', 'src/models/Post.ts')).toBe(false);
  });

  test('handles leading slashes', () => {
    expect(same_module('/src/utils/helpers.ts', 'src/utils/helpers.ts')).toBe(true);
    expect(same_module('/src/models/User.ts', '/src/models/User.js')).toBe(true);
  });
});