...
async function main(): Promise<void> {
  ...
  if (!dry_run && (result.registry_upserts.length > 0 || result.drift_tagged_groups.length > 0)) {
    ...
    // The registry lives at .claude/skills/self-repair-pipeline/known_issues/
    // The bundled builtins barrel + permanent slice live in packages/core/.
    ...
    // Regenerate the bundled permanent slice as a TS module. Filter to
    // status=permanent && classifier.kind != none so library consumers see
    // only rules that actually classify entries. Embedding as TS (rather
    // than a sibling .json) keeps the published bundle self-contained.
    const permanent_slice = registry_after.filter(
      (issue) => issue.status === "permanent" && issue.classifier.kind !== "none",
    );
    const slice_path = path.join(classify_dir, "registry_permanent.ts");
    const slice_body = [
      "/**",
      " * Bundled permanent slice of the known-issues registry.",
      " *",
      " * Generated by the triage-curator's `pnpm sync-permanent-rules` step from",
      " * `.claude/skills/self-repair-pipeline/known_issues/registry.json`, filtered",
      " * to `status: \"permanent\" && classifier.kind !== \"none\"`. Embedded as a TS",
      " * module (rather than a sibling `.json`) so it ships in `dist/` without an",
      " * extra copy step and so the bundle stays self-contained.",
      " */",
      "",
      "import type { KnownIssuesRegistry } from \"@ariadnejs/types\";",
      "",
      `export const PERMANENT_REGISTRY: KnownIssuesRegistry = ${JSON.stringify(
        permanent_slice,
        null,
        2,
      )};`,
    ...
    await fs.writeFile(slice_path, slice_body, "utf8");
    derived_files.push(slice_path);
  }
