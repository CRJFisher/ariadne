...
export function check_callers_not_in_registry_unclassified(
  ...
  const check_3 = (() => { const pattern = new RegExp("^\\s*(this|super)\\.[A-Za-z_][A-Za-z0-9_]*\\s*\\("); return entry.diagnostics.grep_call_sites.some((h) => pattern.test(h.content)); })();
