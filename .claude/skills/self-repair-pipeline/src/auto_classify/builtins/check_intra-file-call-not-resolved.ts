...
export function check_intra_file_call_not_resolved(
  ...
  const check_0 = entry.diagnostics.ariadne_call_refs.length <= 0;
  const check_1 = (entry.diagnostics.grep_call_sites.length > 0 && entry.diagnostics.grep_call_sites.every((h) => h.file_path === entry.file_path)) === true;
  const check_2 = (() => { const pattern = new RegExp("^(?!\\s*(?:export\\s+)?(?:async\\s+)?function\\s)."); return entry.diagnostics.grep_call_sites.some((h) => pattern.test(h.content)); })();
