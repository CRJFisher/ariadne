...
- **False positive** (`ariadne_correct: false`): Real callers exist but resolved to wrong target. `group_id` = kebab-case mismatch type (e.g., `"class-hierarchy-dispatch"`, `"interface-impl-mismatch"`, `"module-shadow-resolution"`, `"overloaded-name-collision"`).
