...
- **False positive** (`ariadne_correct: false`): Real callers exist that resolution failed to link. `group_id` = kebab-case resolution gap (e.g., `"aliased-import-resolution"`, `"barrel-reexport"`, `"prototype-method-dispatch"`, `"generic-type-erasure"`).
