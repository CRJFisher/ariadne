...
classifier from the `plan-strategist` sub-agent: prefer existing
...
registry including the predicate DSL (for context on what hand-authored
entries look like), but sub-agent proposals use the builtin path exclusively.
...
`.claude/skills/plan/src/types.ts`; the renderer rejects any op not
listed there.
...
- **`{ kind: "predicate", axis, expression, min_confidence }`** — Pure
  ...
  `triage/known_issues/` for the axis definitions).
