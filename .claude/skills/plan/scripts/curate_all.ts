#!/usr/bin/env node
/**
 * Default entry for `/plan`.
 *
 * Reads each uncurated `triage_results/<run-id>.json` and partitions its
 * `novel_issues[]` into a read-only dispatch plan the main agent acts on:
 *
 *   - novel issues whose `id` already exists in the registry as a `wip` or
 *     `permanent` row are surfaced as observed-stat bumps for the deferred
 *     actuator (no investigate dispatch).
 *   - novel issues with no matching registry row become a "promote-novel"
 *     dispatch the puller pulls into the investigate wave.
 *   - novel issues matching a `fixed` row are surfaced for human review.
 *
 * This script writes nothing to the registry; it only emits the plan as JSON.
 *
 * Usage:
...
type CliOpts = ScanOptions;

function parse_argv(argv: string[]): CliOpts {
  const opts: CliOpts = { project: null, last: null, run: null };
    ...
    switch (arg) {
      ...
      case "-h":
        process.stdout.write(
          "Usage: curate_all [--project N] [--last N] [--run P]\n",
        );
        process.exit(0);
        break;
      default:
 ...
 *
 *   - `wip` or `permanent` row → surfaced as an observed-stat bump for the
 *     deferred actuator (no investigate dispatch).
...
async function main(): Promise<void> {
  ...
  const settled = await Promise.allSettled(
    items.map((item) =>
      plan_for_run(item, registry_by_group_id, scripts_rel, repo_root),
    ),
  );
  ...
  process.stdout.write(
    JSON.stringify(
      { run_count: dispatches.length, runs: dispatches, failed_runs },
      null,
      2,
    ) + "\n",
  );
