...
describe("reconcile_verdicts", () => {
  ...
  it("records a fixable→permanent flip and leaves an agreeing row alone", () => {
    ...
    expect(reconcile_verdicts(verdicts, flags)).toEqual({ reroutes: expected, unknown_row_ids: [] });
  });
  ...
  it("collects an unknown row id without a reroute", () => {
    ...
    expect(reconcile_verdicts(verdicts, flags)).toEqual({ reroutes: [], unknown_row_ids: ["pt-ghost"] });
  });
});
