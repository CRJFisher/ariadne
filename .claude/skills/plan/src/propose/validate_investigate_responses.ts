 ...
 * `plan-strategist` sub-agent calls
 * `scripts/validate_responses.ts` against each draft inside its
 * propose → validate → iterate loop; the deferred actuator calls
...
/**
 * Validate that a classifier spec's `positive_examples` / `negative_examples`
 * indices are in range against the source group's entry count. Returns a list
 * of human-readable violations (empty when the spec is consistent).
 *
