// Mirror of .claude/skills/triage-curator/src/atomic_write.ts — keep behavior
// identical. The skills are independent pnpm packages with no shared workspace
// layer, so the helper is duplicated rather than imported across package
// boundaries. Any change here must be applied to the mirror.
 ...
 *
 * Used for per-run files with a single declared writer (the dispatcher) where
 * readers in the same process may race the write — e.g. `novel_issues.json`,
 * which the dispatcher rewrites between dispense calls.
 *
