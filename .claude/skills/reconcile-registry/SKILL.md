...
| `drift_on_non_wip_rule_ids` | published `rule_id`s naming a `permanent` or `fixed` rule — drift is a wip-only signal, so no flag is proposed; the rule's predicate is under-matching, so fix its `check_<group_id>.ts` directly or demote the rule back to `wip` by hand. Reported-only, never dropped |
