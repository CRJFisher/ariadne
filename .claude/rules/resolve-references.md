...
- **Value sources** → What a binding holds, when the value flows into it rather than its declaration stating a type: an instance of a type, a class object, or a callable. `call_resolution/value_source.ts` answers it once for four consumers, from producers tried in order: the container element a container read yields; what calling an initialiser's callee yields (`p = parser(io)`, and for Python `initialized_from_call_result`, `p = make()(io)`) — a class object for a declared class-object return, an instance for a declared return type or for a class object called; what a `member_source` member read holds (`orig = BaseTask.__call__`); and a local carrier — a variable's one-name initialiser (`mapper_cls = Mapper`), a class attribute's name or member-read initialiser (`feed_type = feedgenerator.DefaultFeed`), a parameter's default (`Info=TraceInfo`). A carrier never crosses a function boundary, so a class handed in as an argument holds nothing. A scope can bind one name several times and name resolution keeps one of them, so a read at a known position takes the one binding `DefinitionRegistry.get_scope_rebindings` shows preceding it, and holds nothing where several do. The consumers: a receiver identifier with no recorded type takes the type an instance or class object names; a construction whose callee is a binding constructs the class object it holds (`const cls = Parser; new cls()`); a bare call through a binding holding a callable or a class object reaches it, while an instance is left to the Python `__call__` protocol; and a `property_access` read whose value is a function or method marks it indirectly reachable (`self.loop = loops.synloop`) without claiming a call edge for whatever later invokes the stored value.
