...
- **Constructed and call-initialised receivers** → A construction types what stores it, keyed by location: `const t = new T()` the declarator, `x = new T()` a class field, and `this.x = new T()` the field the class body declares — or, in JavaScript, the field that constructor write itself declares when the body declares none. A construction passed as an argument types nothing (`const t = new Outer(new Inner())` holds an `Outer`). A variable with no annotation and no construction takes the declared return type of what its initialiser calls: `initialized_from_call` carries the callee chain root first (`s.getInfo()` → `["s", "getInfo"]`), and `TypeRegistry` resolves it — a self receiver through the self-type lookup the project hands in, a type's static member, a module import's member, or a member of the type already recorded for a value this file declares — to a function or method, whose return annotation is parsed and resolved in its own file. A value another file declares is not followed, so the answer does not depend on the order files resolve in. `type[X]` keeps `type` as the head and `X` as its argument: the binding holds the class object, not an instance.
...
- **Collection dispatch** → Variable holding function collection → member function lookup. A static alias (`var A = Ns.A`, carried as `VariableDefinition.member_source`) dispatches to the one member it names, never the union.
...
│   ├── subtype_graph.ts          # SubtypeGraph (heritage edges: parent → subtypes, subtype → ordered parents, per writing file)
...
- **`DefinitionRegistry`** — Central definition store with indexes by symbol, file, location, scope and member, composing the heritage graph it resolves
- **`TypeRegistry`** — Resolved type relationships: value types, value type arguments, callable return types, type members; walks inheritance over the heritage graph
