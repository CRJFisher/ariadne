...
- **The human owns every write.** Neither pipeline skill writes the registry — `triage` reads it to filter classifier hits; `plan` neither reads nor writes it (its dedup signal is `backlog/tasks/*.md` frontmatter). Every status transition is a human decision, made by hand or through the `reconcile-registry` skill over `reconcile_registry.ts` (`disable-model-invocation: true`; it detects mechanical transitions and proposes every write behind a `--dry-run` preview, never decides). No agent writes unattended: the in-repo `.claude/hooks/registry_write_guard.ts` `PreToolUse` hook routes every agent `Write`/`Edit` to `registry.json` to a per-edit human `ask`, and shell-based writes fall to the harness `[Self-Modification]` classifier.
- **The registry is the permanent-limitations catalog.** Each entry names a call relationship fundamentally unknowable to static analysis — dynamic dispatch through computed keys, runtime invocation via interpreter or framework protocols, bundler module substitution, macro expansion, or callers in unindexed external modules — and carries a real `BuiltinCheckFn` at `packages/core/src/classify_entry_points/builtins/check_<group_id>.ts` that fires during triage. A fixable Ariadne resolution bug is tracked in `backlog/tasks/`, never here.
- **Every write goes through `atomic_update_registry`.** The lock-guarded mutator in `@ariadnejs/skill-fs` (`packages/skill-fs/src/atomic_update_registry.ts`) is the only sanctioned write path; `packages/skill-fs/src/registry_writers.test.ts` enforces it structurally.
- **Agent-initiated transitions route back to the human.** A pipeline agent never self-applies a transition: the `classifier-author` agent stages a draft for `--stage`, and other flows print a `reconcile_registry.ts` command for the human to run (see **Agent-initiated transitions** below). A human directing an interactive session may instead approve a direct registry edit at the permission prompt.
...
When a write throws `could not acquire <path>.lock after 5000ms — stale lock?`, the honest-contention and crashed-writer recovery procedures live in `.claude/skills/reconcile-registry/RUNBOOK.md`.
...
- Stale-lock recovery procedures: `.claude/skills/reconcile-registry/RUNBOOK.md`
