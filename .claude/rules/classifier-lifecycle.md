...
A pipeline agent flow (`build-and-review`, the `classifier-author` drafting agent, any future pipeline agent) does not self-apply a `registry.json` transition as part of its unattended flow. The registry is the human-owned self-modification surface: in auto-accept mode the harness permission classifier denies an agent Bash or Write call targeting `.claude/skills/triage/known_issues/registry.json` as `[Self-Modification]`. A human directing an interactive session can approve such an edit directly — a reviewed human action, e.g. a refactor that converts or retires rules. But a pipeline-originated transition, where no human is watching each write, routes back to the human rather than self-applying — the sanctioned path is a single named command:
