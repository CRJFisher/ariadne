...
A retirement takes two shapes. The **auto-detected** path above is a status-only stamp: the rule flips to `fixed` and its classifier is left intact. The **name-mode** path (`reconcile_registry.ts --id <group_id>... --fixed --reason "<text>"`) is a deliberate retirement for the case the auto detector cannot see — where the subsuming fix landed under a task other than the rule's `backlog_task`, so no git-log scope matches. Name-mode **deletes the named rows** from the registry and unlinks their `check_<group_id>.ts` builtin sources; the `--reason` text survives in the run's summary output and the human's commit message, and git history is the audit trail. Name-mode is guarded: it refuses a row whose `function_name` is still registered in core's `BUILTIN_CHECKS`, so the code-side deletion (barrel entry removed, core rebuilt) must land first. A name-mode deletion of a `permanent` row also regenerates the bundled permanent slice.
