...
tools: Bash(node --import tsx .claude/skills/plan/scripts/get_investigate_context.ts:*), Bash(node --import tsx .claude/skills/plan/scripts/validate_responses.ts:*), Read, Grep, Glob, Write(~/.ariadne/triage-curator/**)
mcpServers:
  - backlog
model: opus
maxTurns: 200
...
done by the time you run. The triage skill's per-entry
...
```bash
node --import tsx .claude/skills/plan/scripts/get_investigate_context.ts \
...
2. **Invoke the validator:**
   ...
   ```bash
   node --import tsx .claude/skills/plan/scripts/validate_responses.ts \
